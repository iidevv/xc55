...
class InputMapper extends Shipping\Mapper\AMapper
{
    ...
    {
        $config = $this->getConfiguration();
        $password = htmlspecialchars($config->password, ENT_XML1 | ENT_QUOTES);
...
XML;
    ...
    protected function getCustomerClassification(): string
    {
        $config = $this->getConfiguration();
        ...
        if (
            $srcCountry === 'US'
            && $config->shipper_number
            && $config->negotiated_rates
        ) {
            $result = <<<XML
<CustomerClassification>
    <Code>00</Code>
...
XML;
    ...
    {
        $dstAddress = $this->inputData['dstAddress'];
        $residentialAddressIndicator = isset($dstAddress['type']) && $dstAddress['type'] === 'R'
