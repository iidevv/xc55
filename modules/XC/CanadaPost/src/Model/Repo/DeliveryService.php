...
namespace XC\CanadaPost\Model\Repo;
...
class DeliveryService extends \XLite\Model\Repo\ARepo
{
    ...
    {
        $service = $this->findOneBy(['code' => $code, 'countryCode' => $countryCode]);
    ...
    protected function collectServiceData($code, $countryCode = '')
    {
        ...
        $data = \XC\CanadaPost\Core\Service\Rating::getInstance()
    ...
    {
        $service = new \XC\CanadaPost\Model\DeliveryService();
        ...
        foreach (['lengthPlusGirthMax', 'lengthHeightWidthSumMax', 'oversizeLimit'] as $k => $v) {
        ...
        foreach ($data->service->options as $_option) {
            $option = new \XC\CanadaPost\Model\DeliveryService\Option();
