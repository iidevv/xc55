...
namespace XC\FacebookMarketing\Core\EventListener;

use XC\FacebookMarketing\Logic\ProductFeed\Generator;

class ProductFeedGeneration extends \XLite\Core\EventListener\Base\Countable
{
    ...
    {
        return parent::isStepValid() && $this->getItems()->valid();
    }
    ...
    protected function finishStep()
    {
        ...
        $this->record['touchData'] = [];
        if ($timeLabel) {
            $this->record['touchData']['timeLabel'] = static::t('About X remaining', ['time' => $timeLabel]);
        }
