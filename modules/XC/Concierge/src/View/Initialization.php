...
use XC\Concierge\Core\Message\Page;
...
class Initialization extends \XLite\View\AView
{
    ...
    public const INTERCOM_HASH_URL = 'https://mc-end-auth.qtmsoft.com/intercom_hash.php';
     ...
     * Get mediator
     *
     * @return \XC\Concierge\Core\Mediator
     */
    protected function getMediator()
    {
        return \XC\Concierge\Core\Mediator::getInstance();
    }
    ...
    protected function defineMessages()
    {
        ...
        if (\XLite\Core\Auth::getInstance()->hasRootAccess()) {
            ...
            $pageMessageIntercom->setIntegrations(['All' => false, 'Intercom' => $this->getMediator()->isIntercomViaSegmentEnabled()]);
            $pageMessages = array_merge($pageMessages, [$pageMessageIntercom->toArray('intercom')]);

            if ($identifyMessages && !empty($identifyMessages[0])) {
                $identifyMessages[0]->setIntegrations(['Intercom' => $this->getMediator()->isIntercomViaSegmentEnabled() ? ['user_hash' => $this->getIntercomHash()] : false]);
            }
        }
