...
class Reviews extends \XLite\Controller\Admin\AAdmin
{
    ...
    public function getItemsListClass()
    {
        return parent::getItemsListClass()
            ?: 'XC\Reviews\View\ItemsList\Model\Review';
    }
    ...
    {
        $dates = $this->getCondition(\XC\Reviews\Model\Repo\Review::SEARCH_ADDITION_DATE);
        $n = ($start === true) ? 0 : 1;
    ...
    protected function doActionUpdateItemsList()
    {
        ...
        if ($productId) {
            /** @var \XC\Reviews\Model\Repo\Review $repo */
    ...
    protected function doActionDelete()
    {
        ...
        if ($select && is_array($select)) {
            \XLite\Core\Database::getRepo('\XC\Reviews\Model\Review')->deleteInBatchById($select);
    ...
    {
        $this->changeReviewStatuses(\XC\Reviews\Model\Review::STATUS_APPROVED);
    }
    ...
    {
        $this->changeReviewStatuses(\XC\Reviews\Model\Review::STATUS_PENDING);
    }
    ...
    protected function changeReviewStatuses($status)
    {
        ...
        if ($select && is_array($select)) {
            ...
            \XLite\Core\Database::getRepo('\XC\Reviews\Model\Review')->updateInBatchById($data);
            \XLite\Core\TopMessage::addInfo(
    ...
    protected function getSearchParams()
    {
        ...
        foreach ($searchParams as $requestParam) {
            if ($requestParam === \XC\Reviews\Model\Repo\Review::SEARCH_ADDITION_DATE) {
    ...
    {
        $qb = \XLite\Core\Database::getRepo('XC\Reviews\Model\Review')
            ->createQueryBuilder()
            ->update();
        ...
        while (
            $ids = array_map(static function ($element) {
