...
class Generator extends \XLite\Base implements \SeekableIterator, \Countable
{
     ...
     */
    public const DEFAULT_TICK_DURATION = 0.5;
    ...
    /**
     * @var \XC\BulkEditing\Logic\BulkEdit\Step\AStep[]
     */
    protected $steps;
     ...
     * Get steps
     *
     * @return \XC\BulkEditing\Logic\BulkEdit\Step\AStep[]
     */
    ...
    {
        if ($this->currentStep === null || $reset) {
            $this->currentStep = $this->defineStep();
        }

        $steps = $this->getSteps();

        return $this->currentStep !== null && isset($steps[$this->currentStep]) ? $steps[$this->currentStep] : null;
    }
    ...
    {
        return [
            'XC\BulkEditing\Logic\BulkEdit\Step\Product',
        ];
    }
