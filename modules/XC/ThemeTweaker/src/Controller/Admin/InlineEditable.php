...
class InlineEditable extends \XLite\Controller\Admin\ACL\Catalog
{
    ...
    protected function doActionUpdateField()
    {
        ...
        // update images
        $images = $nonFiltered['images'] ?? [];
        $videos = $nonFiltered['videos'] ?? [];
        $updates = $nonFiltered['changeset'] ?? [];

        $processedImages = $this->processFieldImageFiles($images, $updates, $errors);
        $processedVideos = $this->processFieldVideoFiles($videos, $updates, $errors);
    ...
    protected function applyUpdate($data)
    {
        ...
        if ($this->validateData($data)) {
            ...
            $entity = \XLite\Core\Database::getRepo($data['model'])->findOneBy([$primaryKey => $data['identifier']]);
    ...
    protected function processFieldImageFiles($files, &$updates, &$errors)
    {
        $processedFiles = [];

        foreach ($files as $fileId) {
            $temporaryFile = $fileId
                ? \XLite\Core\Database::getRepo('\XLite\Model\TemporaryFile')->find($fileId)
                : null;

            if ($temporaryFile) {
                $tempUrl = $temporaryFile->getFrontURL();
                $tempUrl = preg_replace('/(\?|&)t=.*?(&|$)/', '', $tempUrl);
                $file = $this->createContentImageEntity($temporaryFile);

                if ($file) {
                    $newUrl = '/' . str_replace(\XLite\Core\URLManager::getShopURL(), '', $file->getFrontURL());
                    $processedFiles[$fileId] = $newUrl;

                    $updates = array_map(static function ($record) use ($tempUrl, $newUrl) {
    ...
    protected function processFieldVideoFiles($files, &$updates, &$errors)
    {
        $processedFiles = [];

        foreach ($files as $fileId) {
            $temporaryFile = $fileId
                ? \XLite\Core\Database::getRepo('\XLite\Model\Video\Temporary')->find($fileId)
                : null;

            if ($temporaryFile) {
                $tempUrl = $temporaryFile->getFrontURL();
                $tempUrl = preg_replace('/(\?|&)t=.*?(&|$)/', '', $tempUrl);
                $file = $this->createContentVideoEntity($temporaryFile);
                ...
                if ($file) {
                    ...
                    $errors[] = static::t('Error on saving video');
                }
            } else {
                $errors[] = static::t('Unknown temporary file');
            }
    ...
    /**
     * Creates and persists \XLite\Model\Video\Content object from given temporary file.
     *
     * @param   \XLite\Model\Video\Temporary $temporaryFile Temporary file with image
     * @return  \XLite\Model\Video\Content
     */
    protected function createContentVideoEntity($temporaryFile)
    {
        $file = new \XLite\Model\Video\Content();
