 ...
 *
 * @ORM\Entity
 * @ORM\Table (name="vendor_convo_messages")
...
class Message extends \XLite\Model\AEntity
{
    ...
    public const MESSAGE_TYPE_DISPUTE_OPEN  = 'dispute_open';
    public const MESSAGE_TYPE_DISPUTE_CLOSE = 'dispute_close';
     ...
     *
     * @ORM\ManyToOne  (targetEntity="XC\VendorMessages\Model\Conversation", inversedBy="messages")
     ...
     *
     * @ORM\OneToMany (targetEntity="XC\VendorMessages\Model\MessageRead", mappedBy="message", cascade={"all"})
     */
    protected $readers;
     ...
     * Add reader
     *
     * @param \XC\VendorMessages\Model\MessageRead $read
     *
     * @return Message
     */
    ...
    public function markAsRead(\XLite\Model\Profile $profile = null)
    {
        ...
        if (
            ...
            $read = new \XC\VendorMessages\Model\MessageRead();
    ...
    {
        $list = array_filter(array_map(static function ($member) {
    ...
    public function isShouldSendToAdmin()
    {
        if (!$this->getAuthor()->isPermissionAllowed(\XLite\Model\Role\Permission::ROOT_ACCESS)) {
            ...
            if (
                \XC\VendorMessages\Main::isMultivendor()
