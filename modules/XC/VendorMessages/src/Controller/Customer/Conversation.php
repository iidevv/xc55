...
class Conversation extends \XLite\Controller\Customer\ACustomer
{
    ...
    {
        return \XLite\Core\Database::getRepo('XC\VendorMessages\Model\Conversation')
    ...
    protected function doActionContactVendor()
    {
        if (
            ...
            if ($vendor && $profile && $vendor->getProfileId() !== $profile->getProfileId()) {
                $repo = \XLite\Core\Database::getRepo('XC\VendorMessages\Model\Conversation');
                ...
                if (!$conversation) {
                    /** @var \XC\VendorMessages\Model\Conversation $conversation */
