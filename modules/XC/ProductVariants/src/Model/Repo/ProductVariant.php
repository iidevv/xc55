...
class ProductVariant extends \XLite\Model\Repo\ARepo
{
    ...
    public const VARIANT_ID_GENERATION_LIMIT = 50;
     ...
     * Generate SKU
     *
     * @param string|\XC\ProductVariants\Model\ProductVariant $base
    ...
    {
        if ($base instanceof \XC\ProductVariants\Model\ProductVariant) {
            $parts = [];
    ...
    public function getVariantsByAttributeValue(int $productId, int $attributeValueId): array
    {
        return $this->createPureQueryBuilder('v')
            ->linkLeft('v.product', 'p')
            ->where('p.product_id = :productId')
            ->setParameter('productId', $productId)
            ->linkLeft('v.attributeValueS', 'vas')
            ->andWhere('vas.id = :attributeValueId')
