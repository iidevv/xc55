...
namespace QSL\SpecialOffersSpendXGetY\View\Model;
...
class SpecialOffer extends \QSL\SpecialOffersBase\View\Model\ASpecialOffer
{
    ...
    protected function getConditionFields()
    {
        return [
            'sxgyT' => [
                ...
                \XLite\View\FormField\Input\Text\FloatInput::PARAM_MIN => 0.000,
                self::SCHEMA_LABEL      => 'Subtotal (amount to spend)',
                self::SCHEMA_REQUIRED   => false,
                self::SCHEMA_WEIGHT     => 3010,
            ],
            'sxgyConditionCategories' => [
                self::SCHEMA_CLASS      => 'XLite\View\FormField\Select\Categories',
                self::SCHEMA_LABEL      => 'From these categories',
                self::SCHEMA_REQUIRED   => false,
                self::SCHEMA_WEIGHT     => 3520,
            ],
            'sxgyConditionMemberships' => [
                self::SCHEMA_CLASS      => 'QSL\SpecialOffersSpendXGetY\View\FormField\Select\SelectMemberships',
    ...
    protected function getRewardFields()
    {
        return [
            ...
            'sxgyDiscountType' => [
                self::SCHEMA_CLASS      => 'QSL\SpecialOffersSpendXGetY\View\FormField\Select\DiscountType',
    ...
    protected function defineSchemaDefault()
    {
        ...
        $this->schemaDefault['sxgyPromoCategory'] = [
    ...
    {
        $newCategoryIds = isset($data['sxgyConditionCategories'])
            ? array_map('intval', $data['sxgyConditionCategories'])
            : [];

        unset($data['sxgyConditionCategories']);

        parent::setModelProperties($data);

        $entity = $this->getModelObject();
        $this->updateSxgyConditionCategories($entity, $newCategoryIds);
    }
     ...
     * @return void
     */
    protected function updateSxgyConditionCategories($offer, $newCategoryIds)
    {
        $oldCategoryIds = [];
        $c = $offer->getSxgyConditionCategories();
        ...
        if (!empty($oldCategoryOffers)) {
            ...
            if ($categoriesToDelete) {
                ...
                \XLite\Core\Database::getRepo('QSL\SpecialOffersSpendXGetY\Model\CategoryOffer')->deleteInBatch(
                    $categoriesToDelete
                );
            }
        }
        ...
        if ($newCategoryOffers) {
            ...
            \XLite\Core\Database::getRepo('QSL\SpecialOffersBase\Model\SpecialOffer')->update(
                $offer,
                ['sxgyConditionCategories' => $newCategoryOffers]
            );
        }
    }
    ...
    protected function getCategoryOffers($offer, $categories, $categoryIds)
    {
        ...
        foreach ($categories as $category) {
            if (in_array($category->getCategoryId(), $categoryIds)) {
                $links[] = new \QSL\SpecialOffersSpendXGetY\Model\CategoryOffer(
                    [
                        'category' => $category,
                        'offer' => $offer,
                    ]
    ...
    {
        return array_merge(parent::getArrayFieldNames(), ['sxgyConditionMemberships']);
    }
}
