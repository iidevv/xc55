...
use XC\ProductVariants\Model\AttributeValue\AttributeValueSelect;
 ...
 */
abstract class StoreApiProductVariants extends \QSL\CloudSearch\Core\StoreApi
{
        ...
        if ($product->getVariants()->count() > 0) {
            ...
            foreach ($product->getVariants() as $variant) {
                ...
                if (Config::getInstance()->General->show_out_of_stock_products !== 'directLink'
                    && in_array($this->getVariantStockStatus($product, $variant), $this->inStockConditions(), true)
                ) {
                    $availabilityAttr = [
                        'id'                => 'availability',
