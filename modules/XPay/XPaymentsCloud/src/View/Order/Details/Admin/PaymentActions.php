 ...
 * @Extender\Mixin
 */
abstract class PaymentActions extends \XLite\View\Order\Details\Admin\PaymentActions implements \XLite\Base\IDecorator
{
    /**
        ...
        if (isset($transaction) && $transaction->getPaymentMethod()) {
            if ($transaction->isXpayments()) {
                if (in_array(BackendTransaction::TRAN_TYPE_CAPTURE_MULTI, $this->allowedTransactions)) {
                    $this->allowedTransactions = array_diff($this->allowedTransactions, array(
