...
namespace XPay\XPaymentsCloud\Model\Subscription\Base;

use Doctrine\ORM\Mapping as ORM;
use XPay\XPaymentsCloud\Core\Converter;
use XPay\XPaymentsCloud\Core\Translation;
use XPay\XPaymentsCloud\Main as XPaymentsHelper;

/**
 * Subscription Plan base class
 *
 * @ORM\MappedSuperclass
 */
abstract class ASubscriptionPlan extends \XLite\Model\AEntity
{
    /**
     * Plan types
     */
     ...
     */
    const TRIAL_DURATION_UNIT_DAY   = 'D';
    const TRIAL_DURATION_UNIT_WEEK  = 'W';
    const TRIAL_DURATION_UNIT_MONTH = 'M';
    const TRIAL_DURATION_UNIT_YEAR  = 'Y';
    ...
    /**
     * Minimum value of subscription plan trial period
     */
    const MIN_TRIAL_PERIOD_DURATION = 1;
     ...
     * @var float
     *
     * @ORM\Column (type="decimal", precision=14, scale=4, options={ "comment": "Fee for the plan" })
     */
    ...
    public function getNextDate($checkDate = null)
    {
        if (
            ...
            $next = Converter::convertTimeToDayStart(Converter::now() + $this->getTrialOffset());
        } else {
            $next = 0;
     ...
     * @return string
     */
    public function getXpaymentsTrialDescription()
    {
        if (!$this->hasTrialPeriod()) {
            $result = '';
        } else {
            $duration = (int)$this->getTrialDuration();
            $unit = $this->getTrialDurationUnit();
            switch ($unit) {
                case static::TRIAL_DURATION_UNIT_DAY:
                    $unitHuman = (1 === $duration) ? static::t('day') : static::t('days');
                    break;
                case static::TRIAL_DURATION_UNIT_WEEK:
                    $unitHuman = (1 === $duration) ? static::t('week') : static::t('weeks');
                    break;
                case static::TRIAL_DURATION_UNIT_MONTH:
                    $unitHuman = (1 === $duration) ? static::t('month') : static::t('months');
                    break;
                case static::TRIAL_DURATION_UNIT_YEAR:
                    $unitHuman = (1 === $duration) ? static::t('year') : static::t('years');
                    break;
                default:
                    $unitHuman = '';
            }

            $result = $duration . ' ' . $unitHuman;
        }
    ...
    {
        return $this instanceof \XPay\XPaymentsCloud\Model\Subscription\Subscription;
    }
    ...
    {
        return $this instanceof \XPay\XPaymentsCloud\Model\Subscription\Plan;
    }
    ...
    /**
     * Returns the duration of the trial period in seconds
     *
     * @return int
     */
    protected function getTrialOffset()
    {
        $result = 0;
        ...
        switch ($unit) {
            case static::TRIAL_DURATION_UNIT_DAY:
                $result = Converter::addDay($duration, 0);
                break;
            case static::TRIAL_DURATION_UNIT_WEEK:
                $result = Converter::addDay($duration * 7, 0);
                break;
            case static::TRIAL_DURATION_UNIT_MONTH:
                $result = Converter::addMonth($duration, 0);
                break;
            case static::TRIAL_DURATION_UNIT_YEAR:
                $result = Converter::addYear($duration, 0);
                break;
        }
    ...
    {
        return Converter::addMonth($this->getNumber(), $checkDate);
    }
    ...
    {
        return Converter::addYear($this->getNumber(), $checkDate);
    }
    ...
    public function getPlan()
    {
        ...
        return [
            ...
            'period'        => $this->getPeriod(),
            'reverse'       => $this->getReverse(),
            'description'   => $this->getXpaymentsPlanDescription(static::DESCRIPTION_LONG),
        ];
    }
    ...
    public function getTrialPeriod()
    {
        return [
            ...
            'duration_unit' => $this->getTrialDurationUnit(),
        ];
    }
     ...
     * @param array $data
     */
    public function setTrialPeriod(array $data)
    {
        $this->setTrialDuration($data['duration']);
        $this->setTrialDurationUnit($data['duration_unit']);
    }
