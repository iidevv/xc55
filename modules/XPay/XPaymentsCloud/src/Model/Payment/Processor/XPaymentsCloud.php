...
class XPaymentsCloud extends \XLite\Model\Payment\Base\CreditCard
{
     ...
     */
    protected $fraudTransactionTypes = array(
        BackendTransaction::TRAN_TYPE_ACCEPT,
        BackendTransaction::TRAN_TYPE_DECLINE,
    );

    /**
     * PayPal config from X-Payments Cloud instance
     *
     * @var array|null
     */
    ...
    {
        $xpaymentsToken = \XLite\Core\Request::getInstance()->xpaymentsToken;
        $api = $this->initClient();
        ...
        try {
            if ($this->isInitialPaymentTokenizeOnly()) {
                $response = $api->doTokenizeCard(
                    $xpaymentsToken,
                    static::t('Card Setup'),
                    $this->getXpaymentsCustomerId(),
                    $this->prepareCardSetupCart(
                        $this->transaction->getOrder()->getProfile(),
                        $this->transaction->getOrder()->getProfile()->getBillingAddress()
                    ),
            ...
            if (!is_null($response->redirectUrl)) {
                ...
                if (static::SIMPLE_3D_SECURE_MODE) {
                    ...
                    Session::getInstance()->xpaymentsData = [
                        'redirectUrl' => $url,
                        'xpaymentsBuyWithWallet' => \XLite\Core\Request::getInstance()->xpaymentsBuyWithWallet,
                        'xpaymentsWalletId' => \XLite\Core\Request::getInstance()->xpaymentsWalletId,
                    ];
                }
                ...
                if (static::FAILED == $result) {
                    ...
                    $this->processSubscriptions($this->transaction, $payment);
                }

            }
        } catch (\XPaymentsCloud\ApiException $exception) {
            $result = static::FAILED;
            $this->transaction->setDataCell('xpaymentsMessage', $exception->getMessage(), 'Message');

            $note = $this->handleApiException($exception, 'Failed to process the payment!');
        }
    ...
    /**
     * If initial payment should only tokenize card
     *
     * @return boolean
     */
    protected function isInitialPaymentTokenizeOnly()
    {
        ...
        if (!$result) {
            ...
            $result =
                $order->hasOnlyTrialSubscriptionItems()
                && Order::ORDER_ZERO >= $order->getOpenTotal();
        }
    ...
    public function doRebill(Order $order, float $amount, string $xpid)
    {
        ...
        $newTransaction->setPaymentMethod(XPaymentsHelper::getPaymentMethod());
        ...
        try {
            ...
            $result = $this->processPaymentFinish($newTransaction, $payment, true);
            if (static::FAILED == $result) {
                TopMessage::addError($note);
            } else {
                $newTransaction->setStatus(Transaction::STATUS_SUCCESS);
                TopMessage::addInfo($note);
                if ($order->getDelayedPaymentSavedCardId()) {
                    $order->setDelayedPaymentSavedCardId();
                    Database::getEM()->flush();
                    $order->renewPaymentStatus();
                }
            }
    ...
    protected function setTransactionDataCells(Transaction $transaction, XpPayment $payment)
    {
        ...
        if (
            ...
            if (
                ...
                || Format::FORMAT_MASKED === Config::getInstance()->XPay->XPaymentsCloud->card_number_display_format
            ) {
                $first6 = '******';
            } else {
            ...
            if (
                ...
                if (Format::FORMAT_MASKED === Config::getInstance()->XPay->XPaymentsCloud->card_number_display_format) {
    ...
    {
        return XPaymentsHelper::getClient();
    }

    /**
     * Disable wallet methods if disabled
     *
    ...
    {
        return '\XPay\XPaymentsCloud\View\ConnectWidget';
    }
    ...
    {
        $httpsEnabled = Config::getInstance()->Security->admin_security
    ...
    public function processCallback(Transaction $transaction)
    {
        ...
        if ($transaction->isXpayments()) {
            ...
            if (0 !== strcmp($transaction->getXpaymentsId(), $payment->xpid)) {
                // This is a rebill or a new subscription payment
            ...
            if (
                ...
                $transaction->setStatus($payment->isSuccessfulStatus() ? Transaction::STATUS_SUCCESS : Transaction::STATUS_FAILED);
                $this->setTransactionTypeByStatus($transaction, $payment->status);
                $this->setOrderPaymentStatus($transaction->getOrder(), $payment->status);

                if ($payment->isSuccessfulStatus()) {
     ...
     * @return string
     */
    protected function processPaymentFinish(Transaction $transaction, XpPayment $payment, $isRebill = false)
    {
        ...
        if (
            !$isRebill
            && $this->isInitialPaymentTokenizeOnly()
        ) {
            if (
                ...
                if ($order->hasOnlyTrialSubscriptionItems()) {
                    ...
                    $transaction->getOrder()->setDelayedPaymentSavedCardId($savedCardId);
     ...
     */
    public function processCardSetup($token, \XLite\Model\Profile $profile, \XLite\Model\Address $address, $returnUrl, $callbackUrl = '')
    {
        $response = null;
    ...
    /**
     * Prepare array of subscription plans for creating corresponding subscriptions
     *
     * @return array
     */
    protected function prepareSubscriptionPlans(): array
    {
        $result = [];
    ...
    /**
     * Send request to create subscriptions after payment if there were any subscriptions in order
     *
     * @param Transaction                   $transaction
     * @param \XPaymentsCloud\Model\Payment $payment
     *
     * @return void
     */
    protected function processSubscriptions(Transaction $transaction, XpPayment $payment)
    {
        $order = $transaction->getOrder();
        if (
            ...
            try {
                $response = $this->initClient()->doCreateSubscriptions(
     ...
     * @return mixed
     */
    protected function getSavedCardIdByXpid(string $xpid, string $customerId)
    {
        $result = '';
        try {
            $response = $this->initClient()->doGetCustomerCards(
                $customerId
            );
    ...
    /**
     * Checks if subscription, specified in api response actually exists in X-Cart db
     * If not - creates it on the fly
     *
     * @param XpaymentsSubscription $xpaymentsSubscription
     *
     * @return \XPay\XPaymentsCloud\Model\Subscription\Subscription
     */
    protected function persistSubscription(XpaymentsSubscription $xpaymentsSubscription)
    {
        /** @var Subscription $subscription */
        $subscription = Database::getRepo('XPay\XPaymentsCloud\Model\Subscription\Subscription')
            ->findOneBy(['xpaymentsSubscriptionPublicId' => $xpaymentsSubscription->getPublicId()]);
    ...
    protected function createChildTransaction($parentTransaction, XpPayment $payment, XpaymentsSubscription $xpaymentsSubscription = null)
    {
        if ($xpaymentsSubscription) {
            // Fallback order creation if we didn't handled createOrder callback
            $subscription = $this->persistSubscription($xpaymentsSubscription);
            if ($subscription) {
        ...
        if ($transaction) {
            $this->processPaymentFinish($transaction, $payment, true);
        }

        if (XpPayment::INITIALIZED != $payment->status) {
            $transaction->setStatus($payment->isSuccessfulStatus() ? Transaction::STATUS_SUCCESS : Transaction::STATUS_FAILED);
            $this->setOrderPaymentStatus($cart, $payment->status);
        }
        $cart->processSucceed();
    ...
    /**
     * Calculates order payment status based on api response
     *
     * @param Order $order
     * @param int $apiPaymentStatus Payment status from API response
     *
     * @return void
     */
    protected function setOrderPaymentStatus(Order $order, $apiPaymentStatus)
    {
...
HTML;
    ...
    /**
     * Get PayPal config from X-Payments Cloud instance
     *
     * @return array
     */
    public function getPayPalConfig()
    {
        if (null === $this->paypalConfig) {

            try {
                $config = XPaymentsHelper::getClient()->getPaypalConnectionConfig();
            } catch (\XPaymentsCloud\ApiException $exception) {
                $this->log('Error: ' . $exception->getMessage());
            }
            ...
            if (
                !empty($config)
                && $config instanceof \XPaymentsCloud\Response
                && $config->partner_client_id
                && $config->merchant_id_in_paypal
            ) {

                $this->paypalConfig = array(
                    'available'      => true,
                    'connectionInfo' => array(
                        'client_id'     => (string)$config->partner_client_id,
                        'merchant_id'   => (string)$config->merchant_id_in_paypal,
                        'token'         => (string)$config->token,
                        'is_sandbox'    => (bool)$config->is_sandbox,
                        'intent_action' => $config->auth_sale === 'sale' ? 'CAPTURE' : 'AUTHORIZE',
                    ),
                );

            } else {

                // Just in case fill empty values
                $this->paypalConfig = array(
                    'available' => false,
                    'connectionInfo' => array(
                        'client_id'     => '',
