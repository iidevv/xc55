...
abstract class OrderItem extends \XLite\Model\OrderItem implements \XLite\Base\IDecorator
{
     ...
     * @var Subscription
     *
     * @ORM\OneToOne (targetEntity="XPay\XPaymentsCloud\Model\Subscription\Subscription", mappedBy="initialOrderItem", cascade={"all"})
     * @ORM\JoinColumn (name="subscriptionId", referencedColumnName="id", nullable=true, onDelete="SET NULL")
     */
    ...
    */
    ...
    {
        return $this->isXpaymentsSubscription()
            && $this->getProduct()->getXpaymentsSubscriptionPlan()->hasTrialPeriod();
    }
    ...
    public function getXpaymentsTrialDescription()
    {
        return $this->hasTrialPeriod()
            ? $this->getProduct()->getXpaymentsSubscriptionPlan()->getXpaymentsTrialDescription()
            : '';
    }
}
