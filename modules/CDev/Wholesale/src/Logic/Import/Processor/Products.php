...
namespace CDev\Wholesale\Logic\Import\Processor;

use XCart\Extender\Mapping\Extender;
use XLite\Core\Database;
use CDev\Wholesale\Logic\Export\Step\Products as ProductsExport;
...
abstract class Products extends \XLite\Logic\Import\Processor\Products
{
    ...
    {
        foreach (\XLite\Core\Database::getRepo('\CDev\Wholesale\Model\WholesalePrice')->findByProduct($model) as $price) {
            \XLite\Core\Database::getRepo('\CDev\Wholesale\Model\WholesalePrice')->delete($price);
        }

        if ($value) {
            $repo = \XLite\Core\Database::getRepo('CDev\Wholesale\Model\WholesalePrice');
            foreach ($value as $price) {
                if (preg_match('/^(\d+)(-(\d+))?(\((.+)\))?=(\d+\.?\d*)(%?)$/iSs', $price, $m)) {
                    $data = [
                        'membership'         => $this->normalizeValueAsMembership($m[5]),
    ...
    {
        $tiers = [];
        /* @var \CDev\Wholesale\Model\Product $model */
        foreach ($value as $tier) {
            $data = $this->prepareMinPurchaseQuantity($tier);
            ...
            if ($data) {
                $tier = !empty($tiers[$data['membership'] ? $data['membership']->getMembershipId() : null])
                    ...
                    : Database::getRepo('CDev\Wholesale\Model\MinQuantity')->findOneBy([
                        'product'    => $model,
                        'membership' => $data['membership'],
                    ]);

                if (!$tier) {
                    $tier = new \CDev\Wholesale\Model\MinQuantity();
