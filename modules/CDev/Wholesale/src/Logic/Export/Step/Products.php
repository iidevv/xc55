...
namespace CDev\Wholesale\Logic\Export\Step;
...
{
    public const ALL_CUSTOMERS_TIER = 'All customers';
    ...
    protected function getWholesalePricesColumnValue(array $dataset, $name, $i)
    {
        ...
        $cnd->{\CDev\Wholesale\Model\Repo\WholesalePrice::P_PRODUCT} = $dataset['model'];

        return $this->convertWholesalePrices(
            \XLite\Core\Database::getRepo('CDev\Wholesale\Model\WholesalePrice')->search($cnd)
        );
    }
    ...
    protected function convertWholesalePrices(array $prices)
    {
        ...
        /** @var \CDev\Wholesale\Model\Base\AWholesalePrice $price */
