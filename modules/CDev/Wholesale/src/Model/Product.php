...
/**
 * @Extender\Mixin
 * @Extender\Before("XC\ProductVariants")
 */
class Product extends \XLite\Model\Product
{
     ...
     *
     * @ORM\OneToMany (targetEntity="CDev\Wholesale\Model\WholesalePrice", mappedBy="product")
     */
    ...
    {
        $this->applySaleToWholesale = (bool) $applySaleToWholesale;
    }
    ...
    {
        $wholesaleprices = \XLite\Core\Database::getRepo('CDev\Wholesale\Model\WholesalePrice')->getWholesalePrices(
            $this,
    ...
    {
        return \XLite\Core\Database::getRepo('CDev\Wholesale\Model\WholesalePrice')->getPrice(
            $this,
    ...
    {
        foreach (\XLite\Core\Database::getRepo('CDev\Wholesale\Model\MinQuantity')->findBy(['product' => $this]) as $quantity) {
    ...
    {
        foreach (\XLite\Core\Database::getRepo('CDev\Wholesale\Model\WholesalePrice')->findBy(['product' => $this]) as $price) {
