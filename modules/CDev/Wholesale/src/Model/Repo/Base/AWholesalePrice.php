...
namespace CDev\Wholesale\Model\Repo\Base;

use CDev\Wholesale\Model\Base\AWholesalePrice as AWholesalePriceModel;
...
class AWholesalePrice extends \XLite\Model\Repo\ARepo
{
    ...
    public const P_QTY                 = 'quantity';
    public const P_MIN_QTY             = 'minQuantity';
        ...
        if (!empty($membership)) {
            ...
            foreach ($prices as $key => $price) {
                ...
                foreach ($prices as $rangeKey => $range) {
                    if (
                        $price->getQuantityRangeBegin() >= $range->getQuantityRangeBegin()
