...
namespace CDev\USPS\Model\Shipping\Processor;
...
class USPS extends \XLite\Model\Shipping\Processor\AProcessor
{
     ...
     */
    public const USPS_API_DOMESTIC = 'Domestic';
    public const USPS_API_INTL     = 'Intl';
    ...
    {
        return \CDev\USPS\Main::getSettingsForm();
    }
    ...
    public function getApiURL()
    {
        ...
        return $config->test_mode ? Main::getUrlTest() : Main::getUrlLive();
    }
    ...
    protected function prepareDataFromModifier(\XLite\Logic\Order\Modifier\Shipping $inputData)
    {
        ...
        if ($sourceCountry === 'US' || $sourceCountry === 'PR') {
            $result['srcAddress'] = [
            ...
            if ($inputData->getOrder()->getFirstOpenPaymentTransaction()) {
                ...
                if ($paymentMethod && $paymentMethod->getServiceName() === 'COD_USPS') {
    ...
    {
        return $this->getApiType() === self::USPS_API_DOMESTIC
        ...
        try {
            ...
            if (isset($cachedRate)) {
                ...
                if ($response && $response->code == 200) {
                    ...
                    $this->getLogger('CDev-USPS')->debug(
                        '',
                        [
                            'Request URL' => $postURL,
                            'Request XML' => $xmlData,
                ...
                foreach ($response['postage'] as $postage) {
                    ...
                    if ($method && $method->getEnabled()) {
                        ...
                        if (!$this->isStaticCODPrice() && isset($postage['SpecialServices'])) {
                            if (
                                isset($postage['SpecialServices'][6])
                                && $postage['SpecialServices'][6]['Available'] === 'true'
                            ) {
    ...
    {
        [$pounds, $ounces] = $this->getPoundsOunces($data['packages'][$packKey]['weight']);

        $config = $this->getConfiguration();
        $dim = $data['packages'][$packKey]['box'] ?? $data['packages'][$packKey];
    ...
    protected function getXMLDataDomestic($data)
    {
        ...
        foreach ($data['packages'] as $pack) {
            ...
            if (
                !empty($pack['Girth'])
                && 0 < (float) $pack['Girth']
                && in_array($pack['Container'], ['NONRECTANGULAR', 'VARIABLE'], true)
            ) {
                $girth = <<<OUT
                ...
                foreach ($packages as $i => $package) {
                    ...
                    if ($postage) {
                        foreach ($postage as $k => $v) {
                            ...
                            $rateTypes = ['Rate', 'CommercialRate', 'CommercialPlusRate'];
                            $ratePrices = [];
...
OUT;
                ...
                foreach ($packages as $i => $package) {
                    ...
                    if ($postage) {
                        foreach ($postage as $k => $v) {
                            ...
                            // Get rate types returned in response
                            $rateTypes = ['Postage', 'Rate', 'CommercialRate', 'CommercialPlusRate'];
                            $ratePrices = [];
    ...
    {
        $this->apiType = ($address['country'] === 'US' ? self::USPS_API_DOMESTIC : self::USPS_API_INTL);
    }
    ...
    public function isConfigured()
    {
        ...
        return ($config->dataProvider === 'USPS' && $config->userid)
    ...
    protected function getUSPSCountryByCode($code)
    {
        ...
        return ($uspsCountries[$code] ?? null);
    }
    ...
    public static function isCODPaymentEnabled()
    {
        $method = \XLite\Core\Database::getRepo('XLite\Model\Payment\Method')
            ->findOneBy(['service_name' => 'COD_USPS']);
