...
class SelectFile extends \XLite\Controller\Admin\SelectFile
{
    ...
    {
        $productId = (int)\XLite\Core\Request::getInstance()->objectId;

        $product = \XLite\Core\Database::getRepo('XLite\Model\Product')->find($productId);

        if (isset($product)) {
            $attachmentsRepo = \XLite\Core\Database::getRepo('CDev\FileAttachments\Model\Product\Attachment');
            $fileId = (int)\XLite\Core\Request::getInstance()->fileObjectId;

            $attachment = new \CDev\FileAttachments\Model\Product\Attachment();
            ...
            if (call_user_func_array([$attachment->getStorage($methodToLoad), $methodToLoad], $paramsToLoad)) {
                ...
                if (!$found) {
                    foreach ($attachment->getStorage()->getDuplicates() as $duplicate) {
                        if (
                            $duplicate instanceof \CDev\FileAttachments\Model\Product\Attachment\Storage
                ...
                if ($found) {
                    ...
                    /* @var \CDev\FileAttachments\Model\Product\Attachment $attachment_origin */
            ...
            } elseif ($attachment->getStorage()->getLoadError() == 'extension') {
                // Forbid extension
    ...
    protected function getParamsObjectProduct()
    {
        ...
        if (\XLite\Core\Request::getInstance()->fileObject == 'attachments') {
            $result['subpage'] = 'attachments';
        }
    ...
    {
        return \XLite\Core\Database::getRepo('CDev\FileAttachments\Model\Product\Attachment')
