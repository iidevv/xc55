...
class RecentlyViewed extends \XLite\View\ItemsList\Product\Customer\ACustomer
{
     ...
     * Widget parameter
     */
    public const PARAM_PRODUCT_IDS = 'productIds';
    ...
    protected function defineWidgetParams()
    {
        ...
        $this->widgetParams += [
            ...
            self::PARAM_PRODUCT_IDS          => new \XLite\Model\WidgetParam\TypeCollection(
                'ProductIds',
                \XLite\Core\Config::getInstance()->CDev->ProductAdvisor->rv_max_count_in_block === 0
                    ? \CDev\ProductAdvisor\Main::getProductIds()
                    : array_slice(
                        \CDev\ProductAdvisor\Main::getProductIds(),
                        0,
                        \XLite\Core\Config::getInstance()->CDev->ProductAdvisor->rv_max_count_in_block
    ...
    protected function getData(\XLite\Core\CommonCell $cnd, $countOnly = false)
    {
        if ($cnd->{\XLite\Model\Repo\Product::P_PRODUCT_IDS}) {
            ...
            $data = array_map(static function ($i) use ($data) {
                return $data[$i] ?? null;
            }, $order);
