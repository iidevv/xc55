...
namespace CDev\ProductAdvisor\View\Product;
...
class ListItem extends \XLite\View\Product\ListItem
{
    ...
    protected function getLabels()
    {
        ...
        $targets = [
            \CDev\ProductAdvisor\View\ANewArrivals::WIDGET_TARGET_NEW_ARRIVALS,
            \CDev\ProductAdvisor\View\AComingSoon::WIDGET_TARGET_COMING_SOON,
        ];

        if (!in_array($this->getItemListWidgetTarget(), $targets)) {
            // Add ProductAdvisor's labels into the begin of labels list
            $labels = array_reverse($labels);
            $labels += \CDev\ProductAdvisor\Main::getLabels($this->getProduct());
            $labels = array_reverse($labels);
        }
