...
{
    public const SALE_PRICE_LABEL = 'sale_price_label';
    ...
    {
        $oldPrice = \CDev\Sale\Logic\PriceBeforeSale::getInstance()->apply($this->getProduct(), 'getNetPriceBeforeSale', ['taxable'], 'display');
    ...
    public function getSalePriceLabel()
    {
        if (!isset($this->salePriceLabel)) {
            if ($this->participateSale()) {
                ...
                \CDev\Sale\Core\Labels::addLabel($this->getProduct(), $this->salePriceLabel);
            }
        }
    ...
    {
        if ($labelName === static::SALE_PRICE_LABEL) {
