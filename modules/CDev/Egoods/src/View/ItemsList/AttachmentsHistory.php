...
class AttachmentsHistory extends \XLite\View\ItemsList\Model\Table
{
     ...
     */
    public const SORT_BY_MODE_IP    = 'a.ip';
    public const SORT_BY_MODE_LOGIN = 'a.login';
    public const SORT_BY_MODE_DATE  = 'a.date';
    public const SORT_BY_MODE_ORDER = 'a.order';
    public const SORT_BY_MODE_PATH  = 'a.path';
    ...
    public function getCSSFiles()
    {
        return array_merge(
            parent::getCSSFiles(),
            [
                'modules/CDev/Egoods/product/history.less',
            ]
        );
    }
    ...
    {
        return 'CDev\Egoods\View\StickyPanel\AttachmentsHistory';
    }
    ...
    protected function isLink(array $column, \XLite\Model\AEntity $entity)
    {
        $result = parent::isLink($column, $entity)
            && (
                $column[static::COLUMN_CODE] === 'order'
                && $entity->getOrder()
                && $this->isOrderLinkAvailable($entity->getOrder())
                || $column[static::COLUMN_CODE] === 'login'
                && $entity->getProfile()
    ...
    protected function buildEntityURL(\XLite\Model\AEntity $entity, array $column)
    {
        ...
        } elseif ($column[static::COLUMN_CODE] === 'login') {
            return \XLite\Core\Converter::buildURL(
                $column[static::COLUMN_LINK],
                '',
    ...
    {
        return 'CDev\Egoods\Model\Product\Attachment\AttachmentHistoryPoint';
    }
    ...
    protected function postprocessSearchCase(\XLite\Core\CommonCell $searchCase)
    {
        ...
        $searchCase->{\CDev\Egoods\Model\Repo\Product\Attachment\AttachmentHistoryPoint::P_PRODUCT} = $this->getProduct();
