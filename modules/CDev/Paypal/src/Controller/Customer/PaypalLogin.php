...
class PaypalLogin extends \XLite\Controller\Customer\ACustomer
{
    ...
    {
        $loginApi = new \CDev\Paypal\Core\Login();
        $requestProcessed = false;
        $returnURL = '';

        $this->getLogger('CDev-Paypal')->debug('Login return', [
    ...
    protected function getAuthReturnURL($failure = false)
    {
        ...
        if ($controller === 'XLite\Controller\Customer\Checkout') {
            $redirectTo = 'checkout';
        } elseif ($controller === 'XLite\Controller\Customer\Profile') {
            $redirectTo = 'profile';
        }
    ...
    {
        $domains = explode(',', \Includes\Utils\ConfigParser::getOptions(['host_details', 'domains']));
        $domains[] = \Includes\Utils\ConfigParser::getOptions(['host_details', 'http_host']);
        $domains[] = \Includes\Utils\ConfigParser::getOptions(['host_details', 'https_host']);
    ...
    {
        $api = new \CDev\Paypal\Core\Login();
