...
use CDev\Paypal;
use CDev\Paypal\Model\Payment\Processor\PaypalIPN;
...
class PaypalWPS extends \XLite\Model\Payment\Base\WebBased
{
     ...
     * @var string
     */
    protected $knowledgeBasePageURL = 'https://support.x-cart.com/en/articles/5322756-paypal-payments-standard';
    ...
    public function canEnable(\XLite\Model\Payment\Method $method)
    {
        return parent::canEnable($method)
            && $method->getServiceName() == Paypal\Main::PP_METHOD_PPS
    ...
    public function getForbidEnableNote(\XLite\Model\Payment\Method $method)
    {
        ...
        if ($method->getServiceName() == Paypal\Main::PP_METHOD_PPS) {
    ...
    public function processCallback(Transaction $transaction)
    {
        ...
        } elseif (!empty(Request::getInstance()->tx)) {
    ...
    protected function getItems($order)
    {
        ...
        if ($order->countItems()) {
            ...
            $result = array_merge($result, ['items_amount' => $itemsSubtotal]);
    ...
    protected function getStateFieldValue()
    {
        ...
        return $this->getCountryFieldValue() === 'US'
    ...
    protected function getPhone()
    {
        ...
        if ($phone) {
            if (
                ...
                && $this->getProfile()->getBillingAddress()->getCountry()->getCode() == 'US'
            ) {
                $result = [
