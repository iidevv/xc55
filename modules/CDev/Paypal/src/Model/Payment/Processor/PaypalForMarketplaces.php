...
use CDev\Paypal\Core\PaypalForMarketplacesAPI;

class PaypalForMarketplaces extends \CDev\Paypal\Model\Payment\Processor\ExpressCheckout
{
    ...
    /**
     * Prevent enabling 'PayPal For Marketplaces' if Multivendor is not installed and enabled or if Paypal Checkout is enabled
     *
    ...
    {
        $firstTest = parent::canEnable($method)
            && $method->getServiceName() === \CDev\Paypal\Main::PP_METHOD_PFM
            && \Includes\Utils\Module\Manager::getRegistry()->isModuleEnabled('XC', 'MultiVendor');

        if ($firstTest) {
            $paypalCommercePlatform = \CDev\Paypal\Main::getPaymentMethod(\CDev\Paypal\Main::PP_METHOD_PCP);
        }

        return $firstTest && !$paypalCommercePlatform->isEnabled();
    }
    ...
    {
        /** @var \CDev\Paypal\Core\Api\Orders\Order $order */
    ...
    protected function doRefund(\XLite\Model\Payment\BackendTransaction $transaction)
    {
        ...
        foreach ($paymentTransaction->getBackendTransactions() as $backendTransaction) {
            if (
                $backendTransaction->getType() !== $backendTransaction::TRAN_TYPE_SALE
