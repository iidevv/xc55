...
use CDev\Paypal\Core\PaypalCommercePlatform\Convert\CreateOrder;
use CDev\Paypal\Core\PaypalCommercePlatform\GenerateClientToken;
use CDev\Paypal\Core\PaypalCommercePlatform\PaypalClient;
use CDev\Paypal\Core\PaypalCommercePlatform\Webhook;
...
class PaypalCommercePlatform extends \XLite\Model\Payment\Base\Online
{
    ...
    {
        if (
            $method->getServiceName() === \CDev\Paypal\Main::PP_METHOD_PCP
    ...
    public function generateClientToken()
    {
        try {
            ...
            $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: generateToken error', $message);
        }
    ...
    public function createOrder(Transaction $transaction)
    {
        ...
        $response = [];
        try {
            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: createOrder request', $orderData);

            $response = $this->client->execute($request);

            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: createOrder response', [
                'response' => $response,
            ]);
            ...
            $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: createOrder error', $message);
    ...
    public function getPaypalOrder($orderId)
    {
        ...
        $response = [];
        try {
            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: getOrder request', [
                'orderId' => $orderId
            ]);
            ...
            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: getOrder response', [
                'response' => $response,
            ]);
        } catch (\Exception $e) {
            $message = $e->getMessage();
            ...
            $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: getOrder error', $message);
        }
    ...
    {
        $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: onApprove', $data);
    ...
    public function patchOrder(Transaction $transaction)
    {
        ...
        try {
            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: patchOrder request', $operation);

            $response = $this->client->execute($request);

            $response = $response->result;

            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: patchOrder response', [
                'response' => $response,
            ]);
        } catch (\Exception $e) {
            $message = $e->getMessage();
            ...
            $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: patchOrder error', $message);
    ...
    public function captureTransaction(Transaction $transaction)
    {
        ...
        try {
            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: captureTransaction request');

            $response = $this->client->execute($request);

            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: captureTransaction response', [
                'response' => $response,
            ]);
            ...
            foreach ($response->result->purchase_units ?? [] as $purchase_unit) {
                foreach ($purchase_unit->payments->captures ?? [] as $capture) {
                    $captures[] = $capture->id;
                    if ($this->isPurchaseUnitDeclined($status, $capture->status ?? '')) {
                        // force make the transaction failed https://developer.paypal.com/docs/api/orders/v2/#definition-capture_status
                        $status = 'ERROR';
                    }
                }
            }
            ...
            $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: captureTransaction error', $message);
    ...
    public function createAuthorization(Transaction $transaction)
    {
        ...
        try {
            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: createAuthorization request');

            $response = $this->client->execute($request);

            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: createAuthorization response', [
                'response' => $response,
            ]);
            ...
            foreach ($response->result->purchase_units ?? [] as $purchase_unit) {
                foreach ($purchase_unit->payments->authorizations ?? [] as $authorization) {
                    ...
                    if ($this->isPurchaseUnitDeclined($status, $authorization->status ?? '')) {
                        // force make the transaction failed https://developer.paypal.com/docs/api/orders/v2/#definition-authorization_status
                        $status = 'ERROR';
                    }
                }
            }
            ...
            $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: createAuthorization error', $message);
    ...
    public function getCallbackOwnerTransaction()
    {
        ...
        if ($data && $this->webhook->isWebhookData($data)) {
            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: getCallbackOwnerTransaction', $data);
     ...
     *
     * @throws ACallbackException
     */
    public function processCallback(Transaction $transaction)
    {
        $this->transaction = $transaction;

        if (\CDev\Paypal\Model\Payment\Processor\PaypalIPN::getInstance()->isCallbackIPN()) {
            \XLite\Core\Database::getRepo(\XLite\Model\TmpVar::class)->setVar('obsoleteIPNcallbackDetected', \LC_START_TIME);
            // give 200 OK to PayPal to stop these requests
            throw new \XLite\Core\Exception\PaymentProcessing\CallbackRequestError('You have to disable Instant Payment Notification on the page https://www.paypal.com/merchantnotification/ipn/preference/edit as the newest PayPal Checkout doesn\'t need it.');
        }

        PaypalIPN::getInstance()->tryProcessWithLock($transaction, function () use ($transaction) {
            $data = json_decode(file_get_contents('php://input'), true);

            $this->logCallback((array)$data);

            $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: processCallback', $data);
    ...
    protected function doInitialPayment()
    {
        ...
        $fundingData = \XLite\Core\Request::getInstance()->pcp_funding_data;
        if ($fundingData && $fundingData !== 'paypal') {
            $transaction->setDataCell('PaypalFundingSource', ucfirst($fundingData), 'Paypal funding source', \XLite\Model\Payment\TransactionData::ACCESS_CUSTOMER);
        }
        ...
        if (empty($patchResponse)) {
            if ($initialTransactionType === BackendTransaction::TRAN_TYPE_SALE) {
                ...
                if ($response['status']) {
                    ...
                    foreach ($response['captures'] as $captureId) {
                        $transaction->setDataCell('PaypalCaptureID', $captureId, 'Paypal Capture Id', 'C');
                        $backendTransaction->setDataCell('PaypalCaptureID', $captureId, 'Paypal Capture Id');
                        break;
                    }
                }
                ...
                if ($response['status']) {
                    ...
                    foreach ($response['authorizations'] as $authorizationId) {
                        $transaction->setDataCell('PaypalAuthorizationID', $authorizationId, 'Paypal Authorization ID', 'C');
            ...
            if ($response['status']) {
                ...
                } elseif ($response['status'] !== 'VOIDED' && $response['status'] !== 'ERROR') {
                    $transactionStatus = $transaction::STATUS_PENDING;
                    $status            = self::PENDING;
                }
            } else {
    ...
    protected function doCapture(BackendTransaction $backendTransaction)
    {
        ...
        if ($authorizationID) {
            ...
            try {
                $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: doCapture request');

                $response = $this->client->execute($request);

                $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: doCapture response', [
                    'response' => $response,
                ]);

                $transaction->setDataCell('PaypalCaptureID', $response->result->id, 'Paypal Capture Id', 'C');
                ...
                $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: doCapture error', $message);
    ...
    protected function doVoid(BackendTransaction $backendTransaction)
    {
        ...
        if ($authorizationID) {
            ...
            try {
                $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: doVoid request');

                $response = $this->client->execute($request);

                $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: doVoid response', [
                    'response' => $response,
                ]);
                ...
                $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: doVoid error', $message);
    ...
    protected function doRefund(BackendTransaction $backendTransaction)
    {
        ...
        if ($captureID) {
            ...
            try {
                $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: doRefund request', $requestData);

                $response = $this->client->execute($request);

                $this->getLogger('CDev-Paypal')->debug('PaypalCommercePlatform Payment: doRefund response', [
                    'response' => $response,
                ]);
                ...
                $this->getLogger('CDev-Paypal')->error('PaypalCommercePlatform Payment: doRefund error', $message);
     ...
     * @param string $purchaseUnitStatus
     *
     * @return bool
     */
    protected function isPurchaseUnitDeclined(string $mainStatus, string $purchaseUnitStatus)
    {
        return $mainStatus === 'COMPLETED'
            && in_array($purchaseUnitStatus, ['DENIED', 'DECLINED'], true);
    }

    public function get3dsResponse(string $orderId)
    {
        $request = new OrdersGetRequest($orderId);
        $result = null;

        try {
            $this->getLogger('CDev-Paypal')->debug('getOrderPaymentSource request', [ 'orderId' => $orderId ]);

            $response = $this->client->execute($request);

            $authResult = $response->result->payment_source->card->authentication_result ?? null;

            $this->getLogger('CDev-Paypal')->debug('getOrderPaymentSource response', (array)$response->result);
            $this->getLogger('CDev-Paypal')->debug('getOrderPaymentSource Auth result', (array)$authResult);
            ...
            $this->getLogger('CDev-Paypal')->error('getOrderPaymentSource error', $message);
        }
