...
//            \XLite\Model\Payment\BackendTransaction::TRAN_TYPE_REFUND_MULTI
    ...
    protected function getFormFields()
    {
        ...
        if (isset($paypalAdaptiveResponse['payKey'])) {
            ...
            $this->api->doSetPaymentOptionsCall(
                $params['paykey']
            );
        }
    ...
    protected function doRefund(\XLite\Model\Payment\BackendTransaction $transaction, $isDoVoid = false)
    {
        ...
        try {
            ...
            if ($payKey) {
                ...
                $this->getLogger('CDev-Paypal')->error('Adaptive payments error: There is no PAYKEY for refund');
            }
        } catch (\Exception $e) {
            $transaction->setDataCell('errorMessage', $e->getMessage());
            $this->getLogger('CDev-Paypal')->error($e->getMessage());
            \XLite\Core\TopMessage::addError($e->getMessage());
        }

        $transaction->setStatus($backendTransactionStatus);

        return $backendTransactionStatus !== \XLite\Model\Payment\BackendTransaction::STATUS_FAILED;
    }
    ...
    protected function doFullRefund($payKey)
    {
        ...
        if ($result) {
            if (
                isset($result['refundInfoList']['refundInfo'])
                ...
                foreach ($result['refundInfoList']['refundInfo'] as $infoBlock) {
                    if (
                        $infoBlock['refundStatus'] !== 'REFUNDED'
            ...
            $this->getLogger('CDev-Paypal')->error('Adaptive payments error: Refund was failed');
        }
    ...
    public function isConfigured(\XLite\Model\Payment\Method $method)
    {
        return parent::isConfigured($method)
            ...
            && $method->getServiceName() == Paypal\Main::PP_METHOD_PAD
