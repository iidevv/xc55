 ...
 */
class ExpressCheckoutMerchantAPI extends \CDev\Paypal\Model\Payment\Processor\ExpressCheckout
{
    /**
     * API Instance
     *
     * @var \CDev\Paypal\Core\PaypalAPI
     */
    protected $api = null;
    ...
    {
        $this->api = new \CDev\Paypal\Core\PaypalAPI();
    }
    ...
    public function doSetExpressCheckout(\XLite\Model\Payment\Method $method, \XLite\Model\Payment\Transaction $transaction = null)
    {
        ...
        if (!empty($responseData['TOKEN'])) {
            ...
            $this->setDetail(
                'status',
                $responseData['L_LONGMESSAGE0'] ?? 'Unknown',
                'Status'
            );
            ...
            $this->errorMessage = $responseData['L_LONGMESSAGE0'] ?? null;
        }
    ...
    protected function isSuccessResponse($response)
    {
        ...
        if (!$result) {
            $result = (
                $response['PAYMENTINFO_0_PENDINGREASON'] == 'authorization'
    ...
    public function doVoid(\XLite\Model\Payment\BackendTransaction $transaction)
    {
        ...
        return $this->processRequest(
            'DoVoid',
            $transaction,
            static function ($responseData, $state) use ($transaction, $processor) {
