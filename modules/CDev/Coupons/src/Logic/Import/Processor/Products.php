...
namespace CDev\Coupons\Logic\Import\Processor;
...
abstract class Products extends \XLite\Logic\Import\Processor\Products
{
    ...
    {
        return !is_null(\XLite\Core\Database::getRepo('CDev\Coupons\Model\Coupon')->findOneByCode($value));
    }
    ...
    protected function importCouponCodesColumn(\XLite\Model\Product $model, $value, array $column)
    {
        if ($value && is_array($value)) {
            ...
            foreach ($value as $couponCode) {
                $coupon = \XLite\Core\Database::getRepo('CDev\Coupons\Model\Coupon')->findOneByCode($couponCode);
