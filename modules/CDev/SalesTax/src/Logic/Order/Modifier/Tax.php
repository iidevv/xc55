...
namespace CDev\SalesTax\Logic\Order\Modifier;
...
{
    public const MODIFIER_CODE = 'CDEV.STAX';
    ...
    {
        return 'CDev\SalesTax\View\Order\Details\Admin\Modifier\Tax';
    }
    ...
    public static function getSurchargeCode()
    {
        ...
        $taxes = \XLite\Core\Database::getRepo('CDev\SalesTax\Model\Tax')->findActive();

        if ($taxes) {
            // Get the first tax, ignore the rest
            // TODO: Rework this after multi-tax will be removed
            foreach ($taxes as $tax) {
                $result =  \CDev\SalesTax\Logic\Order\Modifier\Tax::MODIFIER_CODE . '.' . $tax->getId();
                break;
            }
        }
    ...
    protected function distributeShippingCost()
    {
        ...
        if ($modifier) {
            ...
            if ($shippingRate && $shippingRate->getMethod()) {
                ...
                $totalAmount = array_reduce($orderItems, static function ($carry, $item) {
        ...
        if ($orderItems && 0 < $totalAmount) {
            // Initialize service variables
            $distributedSum = 0;
            $lastItemKey = null;

            foreach ($orderItems as $key => $item) {
                if ($item->isShippable() && 0 < $totalShippingCost) {
                    ...
                    $orderItems[$key]->setShippingCost(0);
                }
            }
     ...
     *
     * @param \CDev\SalesTax\Model\Tax\Rate $rate Rate
     *
     * @return float
     */
    protected function getShippingTaxCost(\CDev\SalesTax\Model\Tax\Rate $rate)
    {
        $result = 0;
    ...
    {
        return \XLite\Core\Database::getRepo('CDev\SalesTax\Model\Tax')->findActive();
    }
     ...
     * @return array
     */
    protected function getTaxableItems(\CDev\SalesTax\Model\Tax\Rate $rate, array $previousItems = [])
    {
        $list = [];
    ...
    protected function getOrderAddress()
    {
        ...
        if ($profile) {
            $result = \XLite\Core\Config::getInstance()->CDev->SalesTax->addressType === 'shipping'
    ...
    public function getSurchargeInfo(\XLite\Model\Base\Surcharge $surcharge)
    {
        ...
        if (strpos($surcharge->getCode(), $this->code . '.') === 0) {
            $id = (int) substr($surcharge->getCode(), strlen($this->code) + 1);
            $tax = \XLite\Core\Database::getRepo('CDev\SalesTax\Model\Tax')->find($id);
            $info->name = $tax
                ? $tax->getName()
