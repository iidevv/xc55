...
class Magic360ModuleCoreClass
{
     ...
     *
     * @var   \Qualiteam\SkinActMagicImages\Classes\MagicToolboxParamsClass
     *
     */
    public $params;
    ...
    {
        $params = [
            "include-headers-on-all-pages" => ["id" => "include-headers-on-all-pages", "group" => "General", "order" => "10", "default" => "No", "label" => "Include headers on all pages", "description" => "To be able to apply an effect on any page", "type" => "array", "subType" => "radio", "values" => ["Yes", "No"], "scope" => "module"],
            "enable-effect"                => ["id" => "enable-effect", "group" => "General", "order" => "10", "default" => "Yes", "label" => "Enable effect", "type" => "array", "subType" => "radio", "values" => ["Yes", "No"], "scope" => "module"],
            "product-ids"                  => ["id" => "product-ids", "group" => "Magic 360", "order" => "40", "default" => "all", "label" => "Product IDs (all = all products have 360)", "description" => "Choose which products has 360 images (comma separated, e.g. 1,4,5,12,14)", "type" => "text", "scope" => "module"],
            ...
            "thumb-max-width"              => ["id" => "thumb-max-width", "group" => "Positioning and Geometry", "order" => "10", "default" => "300", "label" => "Maximum width of thumbnail for product page (in pixels)", "type" => "num", "scope" => "module"],
            "thumb-max-height"             => ["id" => "thumb-max-height", "group" => "Positioning and Geometry", "order" => "11", "default" => "300", "label" => "Maximum height of thumbnail for product page (in pixels)", "type" => "num", "scope" => "module"],
    ...
    public function getHeadersTemplate($jsPath = '', $cssPath = null)
    {
        ...
        $headers = [];
        // add module version
        $headers[] = '<!-- Magic 360 X-Cart 5 module version v5.4.0 [v1.6.83:v4.6.10] -->';
        $headers[] = '<script type="text/javascript">window["mgctlbx$Pltm"] = "X-Cart 5";</script>';
        // add tool style link
    ...
    public function getMainTemplate($data, $params = [])
    {
        ...
        $rel .= 'rows:' . floor(count($data) / (int) $this->params->getValue('columns')) . ';';
        ...
        $rel .= 'sensitivityX:' . $this->params->getValue('sensitivityX') . ';';
        $rel .= 'sensitivityY:' . $this->params->getValue('sensitivityY') . ';';
        $rel .= 'loop-column:' . $this->params->getValue('loop-column') . ';';
        $rel .= 'loop-row:' . $this->params->getValue('loop-row') . ';';
        $rel .= 'hint:' . $this->params->getValue('hint') . ';';

        if ($this->params->checkValue('hint', true)) {
            $rel .= 'hint-text:' . $this->params->getValue('hint-text') . ';';
            $rel .= 'mobile-hint-text:' . $this->params->getValue('mobile-hint-text') . ';';
        }

        $rel .= 'images:' . implode(' ', $largeImages) . ';';
