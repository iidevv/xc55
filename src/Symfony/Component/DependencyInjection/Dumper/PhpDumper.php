...
namespace XCart\Symfony\Component\DependencyInjection\Dumper;
...
/**
 * It is a copy of the class \Symfony\Component\DependencyInjection\Dumper\PhpDumper (symfony/dependency-injection v5.4.2)
 ...
 */
class PhpDumper extends \Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
...
EOF;
    ...
    private function export($value)
    {
        if (null !== $this->targetDirRegex && \is_string($value) && preg_match($this->targetDirRegex, $value, $matches, \PREG_OFFSET_CAPTURE)) {
            ...
            // In XC development autoloader class may be included by fake path
            // Get correct path from autoloader
            if (
                ($autoloader = AutoloaderManager::getCurrentAutoloader())
                && (
                    ($fqn = $autoloader->suggestFqnByPath($value))
                    || (
                        preg_match('/var\/run\/classes\/(.*)Ancestor\.php/', $value, $m)
                        && ($fqn = str_replace('/', '\\', $m[1]))
                    )
                )
            ) {
                return sprintf("('%s')", $autoloader->getTargetStreamPath($fqn));
            }
