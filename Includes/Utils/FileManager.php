...
abstract class FileManager extends \Includes\Utils\AUtils
{
    ...
    public static function getUniquePath($dir, $file)
    {
        ...
        //https://www.php.net/manual/en/function.pathinfo.php#49765
        $pathinfo['extension'] = preg_match('/\.tar\.gz$/i', $file) ? 'tar.gz' : $pathinfo['extension'];
        $counter  = 1;
    ...
    public static function write($path, $data, $flags = 0, $mode = 0644)
    {
        return static::mkdirRecursive(static::getDir($path))
            ...
            && file_put_contents($path, $data, $flags) !== false;
    }
    ...
    {
        $path = null;
        if (isset($_FILES[$key]) && $_FILES[$key]['error'] === UPLOAD_ERR_OK && static::isDirWriteable($dirTo)) {
    ...
    public static function moveUploadedFileByMultiple($key, $position, $dirTo = LC_DIR_TMP, $name = null, $mode = 0644)
    {
        ...
        if (
            ...
            && $_FILES[$key]['error'][$position] === UPLOAD_ERR_OK
    ...
    public static function findFoldersStartingWithName($root, $name)
    {
        ...
        foreach ($iterator as $fileinfo) {
            if (
                !$fileinfo->isDot()
                && $fileinfo->isDir()
                && ($fileinfo->getFilename() === $name || strpos($fileinfo->getFilename(), $name . '_') === 0)
            ) {
    ...
    {
        $result = null;
        $directories = explode(':', @getenv('PATH'));
    ...
    {
        return ['gif', 'jpg', 'jpeg', 'png', 'ico', 'webp'];
    }
    ...
    public static function isCSV($path)
    {
        ...
        return strtolower($fileInfo['extension']) == 'csv';
    }
    ...
    public static function isZIP($path)
    {
        ...
        return strtolower($fileInfo['extension']) == 'zip';
    }
