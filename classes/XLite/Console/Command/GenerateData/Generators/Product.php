...
class Product
{
    ...
    public function generate($category, $suffix)
    {
        ...
        if (class_exists('CDev\Wholesale\Model\WholesalePrice')) {
    ...
    protected function generateWholesalePrices(\XLite\Model\Product $product, $count)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            $last = $i === $count - 1;
            \XLite\Core\Database::getRepo('CDev\Wholesale\Model\WholesalePrice')->insert(
                [
        ...
        \XLite\Core\Database::getRepo('CDev\Wholesale\Model\MinQuantity')->insert(
            [
                'product'  => $product,
                'quantity' => 1,
            ],
