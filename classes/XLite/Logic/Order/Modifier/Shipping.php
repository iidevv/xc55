...
class Shipping extends \XLite\Logic\Order\Modifier\AShipping
{
     ...
     * Modifier code
     */
    public const MODIFIER_CODE = 'SHIPPING';
        ...
        if (!isset($this->rates[$hash])) {
            if ($this->isCart()) {
                ...
                $rates = array_merge($restored, array_filter(
                    $this->calculateRates(),
                    static function (Rate $rate) use ($restored) {
            ...
            if (!\XLite::isAdminZone()) {
                $this->order->renewShippingMethod();
            }
        }
    ...
    protected function calculateRates()
    {
        ...
        uasort(
            $rates,
            static function (Rate $rateLeft, Rate $rateRight) {
                return $rateLeft->getMethod()->getPosition() <=> $rateRight->getMethod()->getPosition();
            }
        );
     ...
     *
     * @param Rate $rate Shipping rate object OPTIONAL
     *
     * @return void
     */
    public function setSelectedRate(Rate $rate = null)
    {
