...
class Customers extends \XLite\Logic\Import\Processor\AProcessor
{
    ...
    {
        return static::t('https://support.x-cart.com/en/articles/5387451-csv-import-customers');
    }
    ...
    protected function verifyLogin($value, array $column)
    {
        if (!$this->verifyValueAsEmail($value)) {
            $this->addError('USER-LOGIN-FMT', ['column' => $column, 'value' => $value]);
        }
        ...
        if (!$this->isAdminProfilesManager()) {
            ...
            if ($profile && $profile->isAdmin()) {
                $this->addWarning('USER-ADMIN-IGN', ['column' => $column, 'value' => $value]);
    ...
    protected function verifyAdded($value, array $column)
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsDate($value)) {
            $this->addWarning('USER-ADDED-FMT', ['column' => $column, 'value' => $value]);
        }
    }
    ...
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsDate($value)) {
            $this->addWarning('USER-FLOGIN-FMT', ['column' => $column, 'value' => $value]);
        }
    }
    ...
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsDate($value)) {
            $this->addWarning('USER-LLOGIN-FMT', ['column' => $column, 'value' => $value]);
        }
    }
    ...
    protected function verifyStatus($value, array $column)
    {
        if (
            ...
            && !$this->verifyValueAsSet(
                $value,
                [\XLite\Model\Profile::STATUS_ENABLED, \XLite\Model\Profile::STATUS_DISABLED]
            )
        ) {
            $this->addError('USER-STATUS-FMT', ['column' => $column, 'value' => $value]);
        }
    }
    ...
    protected function verifyLanguage($value, array $column)
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsLanguageCode($value)) {
            $this->addWarning('USER-LANGUAGE-FMT', ['column' => $column, 'value' => $value]);
        }
    }
    ...
    protected function verifyShippingAddress($value, array $column, $index)
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsBoolean($value)) {
            $this->addWarning('USER-SHPADDR-FMT', ['column' => $column, 'value' => $value], $index);
        }
    }
    ...
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsBoolean($value)) {
            $this->addWarning('USER-BILADDR-FMT', ['column' => $column, 'value' => $value], $index);
        }
    }
        ...
        if (is_array($value)) {
            foreach ($value as $name => $rows) {
                ...
                if ($this->isColumnHeaderEqual('shippingAddress', $name)) {
                    ...
                    if ($serviceName) {
                        $method = 'verifyAddressField' . \Includes\Utils\Converter::convertToUpperCamelCase($serviceName);
    ...
    protected function verifyAddressFieldCountryCode($value, array $column, $index)
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsCountryCode($value)) {
            $this->addWarning('USER-CCODE-FMT', ['column' => $column, 'value' => $value], $index);
        }
    }
    ...
    protected function verifyAddressFieldStateId($value, array $column, $index)
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsStateId($value)) {
            $this->addWarning('USER-SID-FMT', ['column' => $column, 'value' => $value], $index);
        }
    }
    ...
    protected function verifyMembership($value, array $column)
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsMembership($value)) {
            $this->addWarning('GLOBAL-MEMBERSHIP-FMT', ['column' => $column, 'value' => $value]);
        }
    }
    ...
    {
        if (!$this->verifyValueAsEmpty($value) && !$this->verifyValueAsBoolean($value)) {
            $this->addWarning('USER-FORDER-CHANGE-PASSWORD-FMT', ['column' => $column, 'value' => $value]);
        }
    }
    ...
    protected function verifyAccessLevel($value, array $column)
    {
        if (!$this->verifyValueAsEmpty($value)) {
            if (!$this->isAdminProfilesManager()) {
                ...
                $this->addError('USER-ACCESSLEV-PERM', ['column' => $column]);
            } elseif (!in_array($value, [\XLite\Core\Auth::getInstance()->getAdminAccessLevel(), \XLite\Core\Auth::getInstance()->getCustomerAccessLevel()])) {
                // Wrong value of access level
                $this->addError('USER-ACCESSLEV-FMT', ['column' => $column, 'value' => $value]);
            } else {
                $profile = \XLite\Core\Database::getRepo('XLite\Model\Profile')->findByLogin($this->login);

                if ($profile && (int)$value != (int)$profile->getAccessLevel()) {
                    // It's impossible to change access level of an existing profile
                    $this->addWarning('USER-ACCESSLEV-CHANGE', ['column' => $column, 'value' => $value, 'prevValue' => $profile->getAccessLevel(), 'login' => $profile->getLogin()]);
                }
            }
        }
    }
    ...
    protected function verifyRoles($value, array $column)
    {
        if (!empty($value)) {
            if (!$this->isAdminProfilesManager()) {
                // Administrator hasn't permissions to change access level
                $this->addError('USER-ROLES-PERM', ['column' => $column]);
            } elseif ($this->isCurrentUser) {
                // Admin cannot change its own roles via import
                $this->addWarning('USER-ROLES-SELF', ['column' => $column]);
            } elseif (is_array($value)) {
                foreach ($value as $role) {
                    if (!$this->verifyValueAsEmpty($role) && !$this->verifyValueAsRole($role)) {
                        // Wrong role name
                        $this->addError('USER-ROLE-FMT', ['column' => $column, 'value' => $role]);
                    }
                }
            }
        }
                    ...
                    if ($serviceName) {
                        if ($serviceName == 'state') {
                            ...
                            foreach (\XLite\Core\Database::getRepo('XLite\Model\AddressField')->findAllEnabled() as $field) {
                                $fname = \Includes\Utils\Converter::convertToLowerCamelCase($field->getServiceName());
    ...
    protected function detectModel(array $data)
    {
        ...
        if ($this->isIgnoreRow) {
            $this->addWarning('USER-ADMIN-IMP-IGN', ['value' => $entity->getLogin()]);
