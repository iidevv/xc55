...
class Products extends \XLite\Logic\Import\Processor\AProcessor
{
    ...
    {
        return static::t('https://support.x-cart.com/en/articles/5373610-csv-import-products');
    }
    ...
    protected function verifySku($value, array $column)
    {
        if ($this->verifyValueAsEmpty($value)) {
            ...
            $skuCount = count($this->skuCache);
            if (
                $skuCount === \XLite\Core\EventListener\Import::CHUNK_LENGTH
                || $this->position === ($this->count() - 1)
            ) {
                $this->addLog(\XLite\Model\ImportLog::TYPE_SKU, 'PRODUCT-SKU', $this->skuCache);
            }
        }
    }
        ...
        if ($value) {
            ...
            if (!$this->verifyValueAsNull($value)) {
                foreach ($value as $membership) {
                    ...
                    if ($membership) {
                        $model->addMembershipsByMemberships([$membership]);
                    }
                }
            }
        }
    ...
    protected function importCategoriesColumn(\XLite\Model\Product $model, $value, array $column)
    {
        if ($value && !$this->verifyValueAsNull($value)) {
            ...
            $linksToRemoveKeys = $linksToRemove->map(static function ($link) {
            ...
            array_map(static function ($link) use ($model) {
    ...
    protected function importAttributesColumn(\XLite\Model\Product $model, array $value, array $column)
    {
        ...
        foreach ($value as $attr => $v) {
            if ($attributeStringData = $this->parseAttributeString($attr)) {
                ...
                $attributeGroup = $attributeStringData['attributeGroup'] && $type !== 'product'
                ...
                foreach ($values as $k => $value) {
                    if ($valueStringData = $this->parseAttributeValueString($value)) {
                        $hasOptions = true;
                        array_walk($valueStringData, static function ($valueData, $valueKey) use (&$data, $k) {
