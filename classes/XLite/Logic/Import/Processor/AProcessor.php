...
abstract class AProcessor extends \XLite\Base implements \SeekableIterator, \Countable
{
    ...
    public const COLUMN_HEADER_DETECTOR = 'headerDetector';
    public const COLUMN_VERIFICATOR     = 'verificator';
    public const COLUMN_NORMALIZATOR    = 'normalizator';
    public const COLUMN_IMPORTER        = 'importer';
    public const COLUMN_IS_KEY          = 'isKey';
    public const COLUMN_IS_REQUIRED     = 'isRequired';
    public const COLUMN_IS_MULTIPLE     = 'isMultiple';
    public const COLUMN_IS_MULTIROW     = 'isMultirow';
    public const COLUMN_IS_MULTICOLUMN  = 'isMulticolumn';
    public const COLUMN_IS_MULTILINGUAL = 'isMultilingual';
    public const COLUMN_IS_TAGS_ALLOWED = 'isTagsAllowed';
    public const COLUMN_IS_IMPORT_EMPTY = 'isImportEmpty';
    public const COLUMN_IS_TRUSTED      = 'isTrusted';
    public const COLUMN_PROPERTY        = 'property';
    public const COLUMN_LENGTH          = 'fieldLength';
    public const COLUMN_WEIGHT          = 'weight';

    public const MESSAGE_NOTE     = 'note';
    public const MESSAGE_DECISION = 'decision';

    public const SUBVALUE_DELIMITER = '&&';
    public const NULL_VALUE = 'NULL';

    public const DEFAULT_CHARSET = 'UTF-8';

    public const MAX_PART_FILE_ROWS = 1000;
    ...
    {
        if (!is_array($path)) {
            $path = $path ? array_map('trim', explode('>>>', $path)) : [];
        }
        ...
        while (!$parent && ($node = array_pop($parentPath))) {
        ...
        [$pathCategories, $category] = \XLite\Core\Database::getRepo('XLite\Model\Category')->findAllByPath($path, false, $parent);

        foreach ($pathCategories as $pathCategoryData) {
            $pathCategory = $pathCategoryData['category'] ?? null;
            if (!$pathCategory || $pathCategory->isRootCategory()) {
                continue;
            }

            $parentPath[] = $pathCategoryData['name'];
    ...
    protected function isImportedFile(\SplFileInfo $file)
    {
        ...
        return strpos($file->getFilename(), strtolower(array_pop($parts))) === 0;
    }
    ...
    protected function getFile()
    {
        if (
            ...
            foreach ($this->getFiles() as $p) {
                if ($this->file === null || $found) {
                    $path = $p;
                    break;
                }
                if ($this->file !== null && $p == $this->file->getPathname()) {
                    $found = true;
                }
            }
    ...
    public function next()
    {
        ...
        $this->importer->getOptions()->progressPosition += $this->getProgressPositionIncrement();
        $this->movePointer(1);
    }
     ...
     * @return bool
     */
    public function allowDividedRowsCalculate()
    {
        return true;
    }
        ...
        while (true) {
            ...
            if ($save || $fields === false) {
                ...
                if (!\Includes\Utils\FileManager::write($outputFile, $header . $content)) {
                    $this->getLogger()->debug('Cannot write file ' . $outputFile);
                    $result = [];
                    break;
                }
    ...
    protected function collectRawRows()
    {
        ...
        do {
            ...
            if (!$eof) {
                $rowFile->next();
                $this->position++;
                $this->importer->getOptions()->position += 1;
            }
    ...
    {
        return !is_array($row) || (count($row) === 1 && !(trim($row[0])));
    }
    ...
    {
        return count($rows) === 1 && $this->isColumnHeadersEmpty();
    }
    ...
    protected function processHeaderRow(array $rows)
    {
        ...
        array_walk($row, static function (&$value, $key) {
            $value = trim($value);
        });
    ...
    protected function assembleColumnData(array $column, array $rows)
    {
        ...
        $result = isset($headers[$column[static::COLUMN_NAME]]) ? [] : null;
    ...
    {
        return $this->mode === static::MODE_VERIFICATION;
    }
    ...
    protected function verifyData(array $data)
    {
        ...
        foreach ($this->getColumns() as $column) {
            $this->verifyCell(
                $column,
                $data[$column[static::COLUMN_NAME]] ?? null
            );
        }
    ...
    protected function verifyCell(array $column, $value)
    {
        ...
        if ($this->isColumnRequired($column) && $this->isColumnValueEmpty($column, $value)) {
            ...
            if (!$this->checkValueLength($column, $value)) {
                $this->addWarning('CMN-FIELD-LENGTH', ['column' => $column, 'value' => '']);
            }
        }
    }
    ...
    protected function getPurifiedValue(array $column, $value)
    {
        if (is_array($value)) {
            ...
            if ($orig != $value && !$ignoreWarning) {
                $this->addWarning($wrnType, ['column' => $column, 'value' => '']);
            }
        }
     ...
     * @return boolean
     */
    protected function isIgnoreXSSWarnings($column = [])
    {
        return true;
    }
    ...
    protected function addNoEntityFoundMessage($data = [])
    {
        ...
        if ($keys) {
            $this->addWarning('NO-ENTITY-FOUND', ['keys' => implode('; ', $keys)]);
        }
    }
    ...
    protected function normalizeModelPlainProperty($value, array $column)
    {
        return empty($column[static::COLUMN_NORMALIZATOR])
            ? $value
            : call_user_func([$this, $column[static::COLUMN_NORMALIZATOR]], $value);
    }
     ...
     * @return boolean
     */
    protected function addLog($type, $code, array $arguments = [], $rowOffset = 0)
    {
        $messages = static::getMessages();

        if (empty($messages[$code]) && $type !== \XLite\Model\ImportLog::TYPE_SKU) {
            throw new \InvalidArgumentException('Log code is unknown');
        } else {
            if (!empty($arguments['column']) && is_array($arguments['column'])) {
    ...
    protected function verifyValueAsNull($value)
    {
        ...
        if (is_array($value) && count($value) === 1) {
            ...
            if (is_array($realValue)) {
                ...
                $result = $realValue === static::NULL_VALUE;
            }
        } else {
            $result = $value === static::NULL_VALUE;
        }
    ...
    protected function verifyValueAsCurrencyCode($value)
    {
        ...
        && 0 < \XLite\Core\Database::getRepo('XLite\Model\Currency')->countBy(['code' => $value]);
    }
    ...
    protected function verifyValueAsLanguageCode($value)
    {
        return preg_match('/^[a-z]{2}$/Si', $value)
            && 0 < \XLite\Core\Database::getRepo('XLite\Model\Language')->countBy(['code' => $value]);
    }
    ...
    {
        return preg_match('/^[a-z]{2}$/Si', $value)
            && 0 < \XLite\Core\Database::getRepo('XLite\Model\Country')->countBy(['code' => $value]);
    }
    ...
    protected function verifyValueAsStateId($value)
    {
        return preg_match('/^\d+$/Si', $value)
            && 0 < \XLite\Core\Database::getRepo('XLite\Model\State')->countBy(['state_id' => $value]);
    }
    ...
    {
        return (bool)\XLite\Core\Database::getRepo('XLite\Model\Profile')->findByLogin($value);
    }
    ...
    protected function verifyValueAsProduct($value)
    {
        ...
        return $result ?: \XLite\Core\Database::getRepo('XLite\Model\ImportLog')->isSkuWillBeAdded($value);
    }
    ...
    protected function verifyValueAsFile($value)
    {
        ...
        if (!$this->isVerification() || !$this->importer->getOptions()->ignoreFileChecking) {
            if (\Includes\Utils\FileManager::isReadable(LC_DIR_ROOT . $value)) {
                $result = strpos(realpath(LC_DIR_ROOT . $value), LC_DIR_ROOT) === 0;
    ...
    /**
     * Verify value as stockStatus
     *
     * @param mixed $value Value
     *
     * @return boolean
     */
    protected function verifyValueAsStockStatus($value)
    {
        $stockStatuses = [
            'reduced',
            'increased',
        ...
        return in_array($value, $stockStatuses);
    }
    ...
    {
        return \XLite\Model\Base\Storage::getLocalPathFromURL($path);
    }
    ...
    protected function getImageFilter($image)
    {
         ...
         * @return boolean
         */
        return static function ($element) use ($image) {
            if (
    ...
    {
        return static function ($element) use ($images) {
            return !in_array($element->getHash(), array_map(static function ($image) {
    ...
    protected function processColumns()
    {
        ...
        $weights = array_map(static function ($c) {
    ...
    protected function prepareColumnHandler(array $column, $name, $pattern)
    {
        ...
        if (empty($column[$name])) {
            $uname = \Includes\Utils\Converter::convertToUpperCamelCase($column[static::COLUMN_NAME]);
            $method = str_replace('{name}', $uname, $pattern);
            if (method_exists($this, $method)) {
                $result = $method;
            }
        } elseif ($column[$name] === true) {
            $uname = \Includes\Utils\Converter::convertToUpperCamelCase($column[static::COLUMN_NAME]);
    ...
    protected function detectColumnHeader(array $column, array $row)
    {
        $result = empty($column[static::COLUMN_HEADER_DETECTOR])
            ...
            : call_user_func([$this, $column[static::COLUMN_HEADER_DETECTOR]], $column, $row);

        if (
            !$this->isColumnMultiple($column)
                ...
                if (isset($row[$idx])) {
                    ...
                    if (0 < strlen($value) || $this->isColumnImportEmpty($column)) {
                        if (
                            ...
                            $name = $m[2] ?? $this->importer->getLanguageCode();
                        }
    ...
    protected function setColumnMetaData(array $column, $name, $value)
    {
        ...
        if (!isset($metaData[$column[static::COLUMN_NAME]])) {
            $metaData[$column[static::COLUMN_NAME]] = [];
        }
    ...
    public static function getResultMessage(array $cell)
    {
        ...
        if (!empty($cell['addCount'])) {
            $count += $cell['addCount'];
            $comment[] = static::t('{{count}} created', ['count' => $cell['addCount']]);
        }

        if (!empty($cell['updateCount'])) {
            $count += $cell['updateCount'];
            $comment[] = static::t('{{count}} updated', ['count' => $cell['updateCount']]);
        }
