...
class Categories extends \XLite\Logic\Import\Processor\AProcessor
{
    ...
    {
        return static::t('https://support.x-cart.com/en/articles/5370878-csv-import-categories');
    }
    ...
    protected function verifyIdentity($value, array $column)
    {
        if (!empty($value['categoryId'])) {
            if (!$this->getRepository()->find((int)$value['categoryId'])) {
                $this->addWarning('CATEGORY-CATEGORY-ID-NF', ['column' => $column, 'value' => (int)$value['categoryId']]);
            }
    ...
    protected function verifyName($value, array $column)
    {
        ...
        } elseif (!$this->verifyValueAsUtf8mb3($value)) {
    ...
    protected function detectModel(array $data)
    {
        if (!empty($data['categoryId'])) {
            $model = $this->getRepository()->find((int)$data['categoryId']);
        }
