...
class Products extends \XLite\Logic\Export\Step\Base\I18n
{
    ...
    protected function getAttributesColumns()
    {
        ...
        if ($this->generator->getOptions()->attrs !== 'none') {
            ...
            if ($count) {
                ...
                do {
                    $cnd->{\XLite\Model\Repo\Attribute::P_LIMIT} = [$start, $limit];
                    foreach (\XLite\Core\Database::getRepo('XLite\Model\Attribute')->search($cnd) as $attribute) {
                        ...
                        if ($attribute->getType() === \XLite\Model\Attribute::TYPE_TEXT) {
    ...
    protected function getAttributeColumnValue(array $dataset, $name, $i)
    {
        ...
        if ($attribute->getType() === \XLite\Model\Attribute::TYPE_TEXT) {
            $value = $repo->findOneBy(['product' => $dataset['model'], 'attribute' => $attribute]);
            if ($value) {
                $result[] = $value->getTranslation(substr($name, -2))->getValue();
            }
        } else {
            $values = $repo->findBy(['product' => $dataset['model'], 'attribute' => $attribute]);
    ...
    protected function findProductSpecifiedAttribute($model, $name)
    {
        ...
        return array_reduce($model->getAttributes()->toArray(), static function ($carry, $item) use ($name) {
            return $carry === null && $name === $item->getName()
                ? $item
                : $carry;
        }, null);
    }
    ...
    {
        $attributes = $model->getProductClass() ? $model->getProductClass()->getAttributes()->toArray() : [];

        /** @var \XLite\Model\Attribute $item */
        return array_reduce($attributes, static function ($carry, $item) use ($name, $group) {
            return $carry === null
            && $name === $item->getName()
