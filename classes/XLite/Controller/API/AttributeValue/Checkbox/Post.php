...
namespace XLite\Controller\API\AttributeValue\Checkbox;
...
final class Post
{
    ...
    public function __invoke(AttributeValueCheckbox $data, int $product_id, int $attribute_id): AttributeValueCheckbox
    {
        ...
        if ($attribute->getType() !== Attribute::TYPE_CHECKBOX) {
            throw new \InvalidArgumentException(
                sprintf('Attribute is wrong type - "%s" instead "%s"', $attribute->getType(), Attribute::TYPE_CHECKBOX)
            );
        }
        ...
        if (
            $this->getAttributeValueRepository()->findOneBy(
                [
                    'product'   => $product,
                    'attribute' => $attribute,
                    'value'     => $data->getValue(),
