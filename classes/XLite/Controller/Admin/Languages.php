...
class Languages extends \XLite\Controller\Admin\AAdmin
{
    ...
    protected function doActionUpdateItemsList()
    {
        ...
        if ($defaultCustomerLanguage != \XLite\Core\Config::getInstance()->General->default_language) {
            $lng = \XLite\Core\Database::getRepo('XLite\Model\Language')->findOneBy(['code' => $defaultCustomerLanguage]);
        ...
        if ($defaultAdminLanguage != \XLite\Core\Config::getInstance()->General->default_admin_language) {
            $lng = \XLite\Core\Database::getRepo('XLite\Model\Language')->findOneBy(['code' => $defaultAdminLanguage]);
    ...
    protected function doActionGetCSV()
    {
        ...
        $lng = \XLite\Core\Database::getRepo('XLite\Model\Language')->findOneBy(['code' => $code]);
        ...
        if ($lng) {
            ...
            if ($labels) {
                ...
                \XLite\Core\TopMessage::addWarning('There are no labels translated to X', ['language' => $lng->getName()]);
            }
        } else {
            \XLite\Core\TopMessage::addWarning('Unknown language: X', ['code' => htmlspecialchars($code)]);
        }
