...
class SelectFile extends \XLite\Controller\Admin\AAdmin
{
    ...
    {
        $methodParamsObject = 'getParamsObject' . \Includes\Utils\Converter::convertToUpperCamelCase($this->getObjectTarget());

        return method_exists($this, $methodParamsObject)
            ? call_user_func([$this, $methodParamsObject])
            : [];
    }
    ...
    protected function doActionSelect()
    {
        $actionName = 'doActionSelect'
            . \Includes\Utils\Converter::convertToUpperCamelCase(\XLite\Core\Request::getInstance()->file_select)
            . \Includes\Utils\Converter::convertToUpperCamelCase($this->getObjectTarget())
            . \Includes\Utils\Converter::convertToUpperCamelCase(\XLite\Core\Request::getInstance()->fileObject);
    ...
    protected function doActionSelectImport($methodToLoad, array $paramsToLoad)
    {
        ...
        if ($path) {
            chmod($path, 0644);
            \XLite\Core\Session::getInstance()->importCell = [
                'path'     => $path,
                'position' => 0,
            ];
        }
    ...
    protected function loadFromRequestImport($key)
    {
        ...
        if ($cell) {
            ...
            switch ($cell['error']) {
                ...
                case UPLOAD_ERR_INI_SIZE:
                    ...
                    // $size value will be used in next case
                ...
                case UPLOAD_ERR_PARTIAL:
                    ...
                    // $error value will not be overridden by next case but will be used in last one
                ...
                case UPLOAD_ERR_CANT_WRITE:
                    ...
                    // $error value will be used in next case
        ...
        if ($result && $error) {
            $this->getLogger()->error('Upload file error: ' . $error);
        }
