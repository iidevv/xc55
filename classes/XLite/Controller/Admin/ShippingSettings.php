...
class ShippingSettings extends \XLite\Controller\Admin\AAdmin
{
    ...
    {
        $processorId = \XLite\Core\Request::getInstance()->processor;
    ...
    protected function prepareTestDataFields(array $schema, &$errors)
    {
        ...
        foreach ($schema as $k => $v) {
            ...
            } elseif ($k === 'cod_enabled') {
                $data[$k] = !empty($postedData[$v]);
            } elseif (isset($postedData[$v])) {
                $methodName = 'prepareTestDataField' . \Includes\Utils\Converter::convertToUpperCamelCase($k);
        ...
        foreach (['srcAddress', 'dstAddress'] as $address) {
            ...
            if (isset($data[$address]['state'])) {
                if (!empty($data[$address]['state']) && !empty($data[$address]['country'])) {
                    $isValidState = (bool) \XLite\Core\Database::getRepo('XLite\Model\State')
                        ->findOneByCountryAndCode($data[$address]['country'], $data[$address]['state']);
                }

                if (!$isValidState) {
                    $data[$address]['state'] = $address === 'srcAddress'
    ...
    protected function doActionTest()
    {
        ...
        if (empty($errorFields) && $processor !== null) {
            // Get rates
