...
class PaymentSettings extends \XLite\Controller\Admin\AAdmin
{
    ...
    {
        return array_merge(parent::defineFreeFormIdActions(), ['add', 'switch']);
    }
    ...
    protected function dispatchAJAXEnable()
    {
        if ($this->isAJAX()) {
            $this->printAJAX([
                'href'      => $this->buildURL('payment_settings', 'disable', ['id' => \XLite\Core\Request::getInstance()->id]),
                'removeCSS' => 'disabled',
                'addCSS'    => 'enabled',
            ]);
        }
    ...
    protected function dispatchAJAXDisable()
    {
        if ($this->isAJAX()) {
            $this->printAJAX([
                'href'      => $this->buildURL('payment_settings', 'enable', ['id' => \XLite\Core\Request::getInstance()->id]),
                'removeCSS' => 'enabled',
                'addCSS'    => 'disabled',
            ]);
        }
    ...
    protected function createOfflinePaymentMethod()
    {
        ...
        $title = (string) \XLite\Core\Request::getInstance()->title;
        $instruction = (string) \XLite\Core\Request::getInstance()->getNonFilteredData()['instruction'] ?: null;
        ...
        if ($name) {
            ...
            $method->setClass('XLite\\Model\\Payment\\Processor\\Offline');
            $method->setAdded(true);
