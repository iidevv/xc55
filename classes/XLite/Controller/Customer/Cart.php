...
class Cart extends \XLite\Controller\Customer\ACustomer
{
    ...
    public function getTitle()
    {
        return $this->getCart()->isEmpty()
            ...
            : static::t('Your shopping bag - X items', ['count' => $this->getCart()->countQuantity()]);
    }
    ...
    {
        return (int) \XLite\Core\Request::getInstance()->amount ?: 1;
    }
    ...
    {
        if ($item) {
            $this->prepareCartBeforeAmountChanging();

            return $this->getCart()->addItem($item);
        }
    ...
    protected function prepareCartBeforeAmountChanging()
    {
    ...
    {
        if ($this->getCart()->getAddItemError() === \XLite\Model\Cart::NOT_VALID_ERROR) {
    ...
    /**
     * Process 'Remove item' error
     *
     * @return void
     */
    protected function processRemoveItemError()
    {
        \XLite\Core\TopMessage::addError('Item has not been removed from your cart');
    }

    /**
     * Process 'Remove item' success
     *
     * @param \XLite\Model\OrderItem $item
     *
     * @return void
     */
    protected function processRemoveItemSuccess($item)
    {
        \XLite\Core\TopMessage::addInfo('Item has been removed from your cart');
        \XLite\Core\Event::productRemovedFromCart($this->assembleProductRemovedFromCartEvent($item));
    }
     ...
     * @return array
     */
    protected function assembleProductRemovedFromCartEvent($item)
    {
        return [];
    }
    ...
    protected function getURLToReturn()
    {
        ...
        if (!$url) {
            if (\XLite\Core\Request::getInstance()->returnURL) {
                ...
                $url = $this->buildURL('product', '', ['product_id' => $this->getProductId()]);
            }
        }
