...
class Login extends \XLite\Controller\Customer\ACustomer
{
     ...
     */
    public const SECURE_TOKEN = 'secureToken';
    ...
    public function handleRequest()
    {
        if (
            ...
            && \XLite\Core\Request::getInstance()->{static::PARAM_ACTION} !== 'logoff'
        ) {
    ...
    {
        return \XLite\Core\Request::getInstance()->action !== 'logoff'
    ...
    {
        if ($url = $this->getRedirectFromLoginURL()) {
    ...
    protected function getRedirectFromLoginURL()
    {
        $url = null;
        if (
            $this->profile instanceof \XLite\Model\Profile
    ...
    protected function loginBody()
    {
        ...
        if (!($this->profile instanceof \XLite\Model\Profile)) {
            ...
            $this->getLogger()->warning(sprintf('Log in action is failed (%s)', \XLite\Core\Request::getInstance()->login));
            ...
            if ($this->isNeedFailureRedirect()) {
                ...
                $url = \XLite\Core\URLManager::getShopURL(
                    \XLite\Core\Converter::buildURL('login', '', [], \XLite::getAdminScript())
                );
                $this->setReturnURL($url);
                $this->setHardRedirect(true);
            } else {
    ...
    {
        if (in_array($result, [\XLite\Core\Auth::RESULT_ACCESS_DENIED, \XLite\Core\Auth::RESULT_PASSWORD_NOT_EQUAL])) {
