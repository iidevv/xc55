...
class WithoutProductClassSubExtension implements CollectionSubExtensionInterface, ItemSubExtensionInterface
{
    /**
     * @var string[]
     */
    protected array $operationNames = ['get', 'post', 'put', 'delete'];

    public function support(string $className, string $operationName): bool
    {
        return $className === AttributeGroup::class && in_array($operationName, $this->operationNames, true);
    }
    ...
    protected function addWhere(QueryBuilder $queryBuilder): void
    {
        ...
        $queryBuilder->andWhere(sprintf('%s.productClass IS NULL', $rootAlias));
    }
}
