...
abstract class Address extends \XLite\Model\AEntity
{
     ...
     * @var array
     */
    protected static $addressFieldsCache = [];
     ...
     * @var array
     */
    protected static $defaultFieldValuesCache = [];
     ...
     *
     * @ORM\ManyToOne (targetEntity="XLite\Model\State", cascade={"merge","detach"})
     * @ORM\JoinColumn (name="state_id", referencedColumnName="state_id", onDelete="SET NULL")
     */
    protected $state;
     ...
     *
     * @ORM\ManyToOne (targetEntity="XLite\Model\Country", cascade={"merge","detach"})
     * @ORM\JoinColumn (name="country_code", referencedColumnName="code", onDelete="SET NULL")
     */
    protected $country;
    ...
    {
        $this->setCountry(
            \XLite\Core\Database::getRepo('XLite\Model\Country')->findOneBy(['code' => $countryCode])
        );
    }
    ...
    public function getStateId($strict = false)
    {
        ...
        return $result ?? (!$strict ? static::getDefaultFieldPlainValue('state_id') : null);
    }
     ...
     */
    protected static function getAddressFieldByServiceName($fieldName)
    {
        if (empty(static::$addressFieldsCache[$fieldName])) {
            array_map(static function (AddressField $field) {
    ...
    {
        return $this->getFieldsAlternativeNames()[$name] ?? [];
    }
        ...
        foreach ($this->getRequiredFieldsByType($atype) as $name) {
            ...
            if (!strlen($this->$method())) {
                foreach ($this->getFieldAlternativeNames($name) as $getter) {
                    $method = 'get' . \Includes\Utils\Converter::convertToUpperCamelCase($getter);
                    if (strlen($this->$method())) {
                        $valid = true;
                        break;
                    }
    ...
    {
        return count($this->getRequiredEmptyFields($atype)) == 0;
    }
