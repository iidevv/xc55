...
abstract class Storage extends \XLite\Model\AEntity
{
     ...
     * Storage type codes
     */
    public const STORAGE_RELATIVE = 'r';
    public const STORAGE_ABSOLUTE = 'f';
    public const STORAGE_URL      = 'u';
    ...
    public function getURL()
    {
        ...
        if ($this->isURL()) {
            ...
            $url = \XLite::getInstance()->getShopURL(
                $webRootPrefix . $this->getWebRoot() . $this->convertPathToURL($path),
    ...
    public function loadFromRequest($key)
    {
        ...
        if ($path) {
            ...
            $this->getLogger()->error('The file was not loaded');
        }
        ...
        if (\Includes\Utils\FileManager::isExists($path) && $this->checkSecurity($path)) {
            ...
            if (empty($local)) {
                ...
                if (\Includes\Utils\FileManager::copy($path, $newPath)) {
                    ...
                    $this->getLogger()->error('\'' . $path . '\' file could not be copied to a new location \'' . $newPath . '\'.');
        ...
        if ($remoteResource->isAvailable()) {
            try {
                ...
                if (\XLite\Core\Operator::writeURLContentsToFile($url, $tmp)) {
                    ...
                    $this->getLogger()->error('Unable to write data to file \'' . $tmp . '\'.');
                }
            } catch (\Exception $e) {
                \Includes\Utils\FileManager::deleteFile($tmp);
                ...
                $this->getLogger()->error('Unable to download the contents of \'' . $url . '\'.');
            }
        } else {
            ...
            $this->getLogger()->error('Unable to get at the contents of \'' . $url . '\'.');
        }
    ...
    {
        $webDir = ltrim(\Includes\Utils\ConfigParser::getOptions(['host_details', 'web_dir']) . '/', '/');
    ...
    public function getStoragePath($path = null)
    {
        ...
        if ($this->getStorageType() == static::STORAGE_RELATIVE) {
            if ($path && strpos($path, $this->getFileSystemRoot()) === 0) {
                $result = $path;
            } else {
                $result = $this->getFileSystemRoot() . ($path ?: $this->getPath());
            }
        } elseif ($this->getStorageType() == static::STORAGE_ABSOLUTE) {
            $result = ($path ?: $this->getPath());
        } elseif ($this->getStorageType() == static::STORAGE_URL) {
            $result = $this->getPath();
        }
    ...
    protected function savePath($path, $pathAsIs = false)
    {
        ...
        // Remove old file
        $savePath = $pathAsIs || $this->getStorageType() == static::STORAGE_ABSOLUTE
            ? $path
    ...
    protected function getLocalPath()
    {
        ...
        if ($this->isURL()) {
            ...
            if (!\Includes\Utils\FileManager::write($path, $this->getBody())) {
                $this->getLogger()->error('Unable to write data to file \'' . $path . '\'.');
                $path = false;
            }
            $isTempFile = true;
        } else {
