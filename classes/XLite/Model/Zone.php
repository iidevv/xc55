 ...
 *      indexes={
 *          @ORM\Index (name="zone_name", columns={"zone_name"}),
 *          @ORM\Index (name="zone_default", columns={"is_default"})
 *      }
 * )
 */
class Zone extends \XLite\Model\AEntity
{
     ...
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\ZoneElement", mappedBy="zone", cascade={"all"})
     */
     ...
     * @var \Doctrine\Common\Collections\ArrayCollection
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\Shipping\Markup", mappedBy="zone", cascade={"all"})
     */
    ...
    public function getZoneStates($excluded = false)
    {
        ...
        if (!empty($stateCodes) || $excluded) {
            ...
            usort($allStates, ['\XLite\Model\Zone', 'sortStates']);
        ...
        if (
            ...
            foreach ($elements as $code) {
                if ($code === $address['country'] . '_' . $state) {
                    $found = true;
                    break;
                } elseif (strpos($code, $address['country'] . '_') === 0) {
                    $need = true;
                }
            }
        }
