...
class Config extends \XLite\Model\Repo\Base\I18n
{
    ...
    public function getByCategory($category, $force = false, $doNotProcess = false)
    {
        ...
        if (!$force) {
            $data = $this->getFromCache('category', ['category' => $category]);
        }
        ...
        if ($data === null) {
            ...
            if (!$doNotProcess) {
                ...
                $this->saveToCache($data, 'category', ['category' => $category]);
            }
        }
    ...
    public function findAddressOptions()
    {
        ...
        if ($result) {
            foreach ($result as $k => $v) {
                if (
                    strpos($v->getName(), 'origin_') === false
                    && strpos($v->getName(), 'location_') === false
                ) {
                    unset($result[$k]);
                }
            }
    ...
    protected function isOptionVisible(\XLite\Model\Config $option)
    {
        ...
        $method = 'isOption' . \Includes\Utils\Converter::convertToUpperCamelCase($option->getCategory() . '_' . $option->getName()) . 'Visible';
        if (method_exists($this, $method)) {
            // Call method 'isOption<Category><Name>Visible'
            $result = $this->$method();
        }

        $notOrigin = (strpos($option->getName(), 'origin_') === false);
    ...
    public function loadFixtures(array $data, \XLite\Model\AEntity $parent = null, array $parentAssoc = [])
    {
        ...
        $this->saveToCache(null, static::CACHE_KEY);
    }
     ...
     * @return void
     */
    public function updateOptionInCache(\XLite\Model\Config $option, $params = [])
    {
        /** @var ConfigCell $data */
        $data = $this->getFromCache(static::CACHE_KEY, $params);
        ...
        $this->saveToCache($data, static::CACHE_KEY, $params);
    ...
    public function processOptions($data)
    {
        ...
        foreach ($data as $option) {
            ...
            $isModuleConfig = strpos($category, '\\') !== false;

            if ($isModuleConfig) {
                // Process module config

                [$author, $module] = explode('\\', $category);
        ...
        if (isset($config->General)) {
            if (isset($config->General->default_state)) {
            ...
            // Type cast
            if (isset($config->General->minimal_order_amount)) {
                $config->General->minimal_order_amount = (float) $config->General->minimal_order_amount;
            }
            if (isset($config->General->maximal_order_amount)) {
    ...
    public function createOption($data, $silent = false)
    {
        ...
        if (!empty($fields['type']) && !$this->isValidOptionType($fields['type'])) {
            throw new \Exception('createOptions() failed: Wrong option type: ' . $fields['type'] . var_export($data));
        }
    ...
    {
        return $this->findOneBy(['name' => $data['name'], 'category' => $data['category']]);
    }
    ...
    protected function defineCacheCells()
    {
        ...
        $list['all'] = [];
        $list['category'] = [
            static::ATTRS_CACHE_CELL => ['category']
        ];
    ...
    protected function getOptionsAvailabilityCondition()
    {
        ...
        foreach ($this->disabledOptions as $category => $options) {
            ...
            [$keys, $options] = \XLite\Core\Database::prepareArray($options, $category);
    ...
    {
        [$condition, $params] = $this->getOptionsAvailabilityCondition();
    ...
    protected function isValidOptionType($optionType)
    {
        ...
        if (
            !$simple
            && (
                preg_match('/^XLite\\\View\\\FormField\\\/S', $optionType)
                || preg_match('/^.+\\\.+\\\View\\\FormField\\\/S', $optionType)
            )
        ) {
            $simple = true;
        }
