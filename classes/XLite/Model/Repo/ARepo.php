...
use XCart\Container;
use XCart\Doctrine\FixtureLoader\YamlLoadingOptions;
...
abstract class ARepo extends \Doctrine\ORM\EntityRepository
{
     ...
     */
    public const KEY_TYPE_CACHE_CELL  = 'keyType';
    public const ATTRS_CACHE_CELL     = 'attrs';
    public const RELATION_CACHE_CELL  = 'relation';
    public const CONVERTER_CACHE_CELL = 'converter';
    public const GENERATOR_CACHE_CELL = 'generator';

    /**
     * Cache key types
     */
    public const CACHE_ATTR_KEY   = 'attributesKey';
    public const CACHE_HASH_KEY   = 'hashKey';
    public const CACHE_CUSTOM_KEY = 'customKey';

    public const DEFAULT_KEY_TYPE = self::CACHE_ATTR_KEY;
    public const EMPTY_CACHE_CELL = 'all';
     ...
     */
    public const FLUSH_BY_DEFAULT = true;

    /**
     * Name of the prefix for the calculated field
     */
    public const CALCULATED_FIELD_FLAG       = 'calculatedField.';
    public const CALCULATED_FIELD_ALIAS_FLAG = 'calculatedFieldAlias.';

    /**
     * Search conditions
     */
    public const P_LIMIT    = 'limit';
    public const P_ORDER_BY = 'orderBy';

    /**
     * Search types
     */
    public const SEARCH_MODE_COUNT    = 'count';
    public const SEARCH_MODE_ENTITIES = 'entities';
    public const SEARCH_MODE_INDEXED  = 'searchModeIndexed';
    public const SEARCH_MODE_IDS      = 'ids';

    /**
     * Any search mode alias for excluding contidions
     */
    public const EXCLUDE_FROM_ANY = 'excludeFromAny';
    ...
    protected YamlLoadingOptions $yamlLoadingOptions;
    ...
    public function __construct(EntityManagerInterface $em, \Doctrine\ORM\Mapping\ClassMetadata $class)
    {
        ...
        $this->yamlLoadingOptions = Container::getContainer()->get('XCart\Doctrine\FixtureLoader\YamlLoadingOptions');
    ...
    public function addCacheRelations(array $externalCells)
    {
        ...
        \XLite\Model\Repo\ARepo::getCacheDriver()->save($key, $cacheCells);
    }
    ...
    public function getRelatedCacheCells()
    {
        ...
        return is_array($cacheCells) ? $cacheCells : [];
    }
    ...
    public function deleteCacheByEntity(\XLite\Model\AEntity $entity)
    {
        foreach ($this->getCacheCells() as $name => $cell) {
            ...
            if ($cell[static::CONVERTER_CACHE_CELL]) {
                ...
                is_array($cell[static::ATTRS_CACHE_CELL])
                && $cell[static::ATTRS_CACHE_CELL]
            ) {
                $params = [];
                foreach ($cell[static::ATTRS_CACHE_CELL] as $key) {
                    $params[$key] = $entity->{'get' . \Includes\Utils\Converter::convertToUpperCamelCase($key)}();
                }
            } else {
                $params = [];
            }
    ...
    public function find($id, $lockMode = null, $lockVersion = null)
    {
        ...
        return $id !== null ? parent::find($id, $lockMode, $lockVersion) : null;
    }
     ...
     * @return void
     */
    public function updateById($id, array $data = [], $flush = self::FLUSH_BY_DEFAULT)
    {
    ...
    {
        if ($entity === null || !($entity instanceof \XLite\Model\AEntity)) {
    ...
    public function getFieldInfo($field, $param = null)
    {
        ...
        return \Includes\Utils\ArrayManager::getIndex($result, $param, $param !== null);
    }
     ...
     * @param \XLite\Model\AEntity|null $parent      Entity parent callback OPTIONAL
    ...
    public function loadFixtures(array $data, \XLite\Model\AEntity $parent = null, array $parentAssoc = [])
    {
        foreach ($data as $record) {
            $this->loadFixture($record, $parent, $parentAssoc);
        }
    }
    ...
    public function loadFixture(
        ...
        $insert = $this->yamlLoadingOptions->getOption('insert');
        ...
        $addModel   = $this->yamlLoadingOptions->getOption('addModel');
        $isAddModel = false;
        if ($addModel === $this->_class->name) {
            ...
            $addModel
            && !$entity
            && !$this->yamlLoadingOptions->getOption('isAddModel')
            && !$this->yamlLoadingOptions->getOption('addParent')
        ) {
            return null;
        }
        ...
        if ($isAddModel) {
            $this->yamlLoadingOptions->setOption('isAddModel', true);
        }

        $this->loadRawFixture($entity, $record);

        if ($isAddModel) {
            $this->yamlLoadingOptions->setOption('isAddModel', null);
        }
     ...
     * @return void
     */
    public function loadRawFixture(\XLite\Model\AEntity $entity, array $record)
    {
        ...
        foreach ($this->assembleAssociationsFromRecord($record, $assocs) as $name => $value) {
            $repo = \XLite\Core\Database::getRepo($assocs[$name]['entityName']);
            if ($assocs[$name]['many']) {
                $repo->loadFixtures($value, $entity, $assocs[$name]);
            } else {
                $repo->loadFixture($value, $entity, $assocs[$name]);
            }
        }
    }
    ...
    public function getEntityProperties()
    {
        if ($this->entityProperties === null) {
            ...
            foreach ($cmd->fieldMappings as $f => $mapping) {
                $fCamelCase  = \Includes\Utils\Converter::convertToUpperCamelCase($f);
                $regular[$f] = [
            ...
            foreach ($cmd->associationMappings as $f => $fData) {
                ...
                $assoc = [
                    'many'         => $isMany,
                    'many2many'    => $fData['type'] === \Doctrine\ORM\Mapping\ClassMetadataInfo::MANY_TO_MANY,
                    'getter'       => 'get' . $fCamelCase,
                ...
                foreach (\XLite\Core\Database::getEM()->getClassMetadata($targetEntity)->identifier as $ident) {
                    $identCamelCase               = \Includes\Utils\Converter::convertToUpperCamelCase($ident);
                    $assoc['identifiers'][$ident] = [
                ...
                if ($fData['mappedBy']) {
                    $mappedCamelCase       = \Includes\Utils\Converter::convertToUpperCamelCase($fData['mappedBy']);
            ...
            $this->entityProperties = [$regular, $assocs, $identifiers];
        }
     ...
     * @return integer
     */
    public function unloadFixtures(array $data, \XLite\Model\AEntity $parent = null, array $parentAssoc = [])
    {
        $result = 0;
    ...
    {
        array_map([$this, 'deleteCache'], array_keys($this->defineCacheCells()));
    }
    ...
    protected function postprocessCacheCells(array $cacheCells)
    {
        ...
        foreach ($cacheCells as $name => $cell) {
            ...
            if (!isset($cell[static::RELATION_CACHE_CELL])) {
                $cell[static::RELATION_CACHE_CELL] = [];
            }
            ...
            if ($this->cacheCells[$name][static::KEY_TYPE_CACHE_CELL] ?? static::CACHE_CUSTOM_KEY === null) {
     ...
     * @return mixed|void
     */
    protected function getFromCache($name, array $params = [])
    {
        $result = null;
        $cell   = $this->getCacheCells($name);
     ...
     * @return void
     */
    protected function saveToCache($data, $name, array $params = [])
    {
        $cell = $this->getCacheCells($name);
    ...
    protected function getCellHash($name, array $cell, array $params)
    {
        ...
        if ($cell[static::KEY_TYPE_CACHE_CELL] === static::CACHE_ATTR_KEY) {
            $hash = implode('.', $params);
        } elseif ($cell[static::KEY_TYPE_CACHE_CELL] === static::CACHE_HASH_KEY) {
            $hash = md5(implode('.', $params));
        } elseif ($cell[static::KEY_TYPE_CACHE_CELL] === static::CACHE_CUSTOM_KEY) {
    ...
    {
        return 'getCacheHash' . \Includes\Utils\Converter::convertToUpperCamelCase($name);
    }
    ...
    {
        return 'convertRecordToParams' . \Includes\Utils\Converter::convertToUpperCamelCase($name);
    }
    ...
    protected function getQueryBuilder()
    {
        if ($this->queryBuilderClass === null) {
            ...
            if (!class_exists($this->queryBuilderClass)) {
    ...
    public function setExportFilter($filter)
    {
        if (!empty($filter)) {
            ...
            $ids = !empty($this->exportSelection)
                ? $this->exportSelection
                : $this->search($filter, static::SEARCH_MODE_IDS);
    ...
    protected function isConditionExcluded($conditionName)
    {
        ...
        $excludeCondition = $excludedConditions[$conditionName] ?? false;
    ...
    protected function processExpressionSearchCondition(\XLite\Model\SearchCondition\IExpressionProvider $condition)
    {
        ...
        if (!isset($this->searchState['parts'][$cndType])) {
            $this->searchState['parts'][$cndType] = [];
        }
    ...
    {
        $methodName = 'prepareCnd' . \Includes\Utils\Converter::convertToUpperCamelCase($key);
        ...
        if (method_exists($this, $methodName)) {
            ...
            $this->getLogger()->debug(
                ...
                [
                    'repo'                 => get_class($this),
                    'condition'            => $key,
                    'search() called from' => $this->getSearchCaller(),
                ]
            );
        }
    }
    ...
    {
        $this->defineRemoveByModuleQuery($module)->execute();
    }

    protected function defineRemoveByModuleQuery(string $module): \XLite\Model\QueryBuilder\AQueryBuilder
    {
        $qb    = $this->createPureQueryBuilder();
        $alias = $this->getMainAlias($qb);
        ...
        $qb
            ...
            ->andWhere("{$alias}.module = :moduleId")
