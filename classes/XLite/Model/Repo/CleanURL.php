...
class CleanURL extends \XLite\Model\Repo\ARepo
{
    ...
    public const CATEGORY_URL_FORMAT_NON_CANONICAL_NO_EXT = 'domain/parent/goalcategory/';
    public const CATEGORY_URL_FORMAT_CANONICAL_NO_EXT = 'domain/goalcategory/';
    public const CATEGORY_URL_FORMAT_NON_CANONICAL_EXT = 'domain/parent/goalcategory.html';
    public const CATEGORY_URL_FORMAT_CANONICAL_EXT = 'domain/goalcategory.html';

    public const PRODUCT_URL_FORMAT_NO_EXT = 'domain/goalproduct';
    public const PRODUCT_URL_FORMAT_EXT = 'domain/goalproduct.html';

    public const PRODUCT_URL_CANONICAL_WITH_CATEGORY = 'domain/main_category_clean_url/product_clean_url';

    /**
     * Limit of iterations to generate clean URL
     */
    public const CLEAN_URL_CHECK_LIMIT = 1000;

    /**
     * Use this char as separator, if the default one is not set in the config
     */
    public const CLEAN_URL_DEFAULT_SEPARATOR = '-';

    /**
     * Default extension
     */
    public const CLEAN_URL_DEFAULT_EXTENSION = 'html';
    ...
    public function getPattern($entity)
    {
        ...
        $method = $entityType
            ? __FUNCTION__ . \Includes\Utils\Converter::convertToUpperCamelCase($entityType)
            : null;
    ...
    public function generateCleanURL(\XLite\Model\AEntity $entity, $base = null)
    {
        ...
        if ($base) {
            ...
            while (
                !$this->isURLUnique($this->postProcessURL($result . $suffix, $entity), $entity)
    ...
    protected function getURLBase($entity)
    {
        ...
        return $result ?? $entity->{$this->getBaseFieldName($entity)};
    }
    ...
    {
        return (array) \Includes\Utils\ConfigParser::getOptions(['clean_urls', 'aliases']);
    }
    ...
    protected function findEntityByURL($entityType, $url)
    {
        ...
        $method = $entityType
            ? 'get' . \Includes\Utils\Converter::convertToUpperCamelCase($entityType)
            : null;
    ...
    public function buildURL($target = '', array $params = [])
    {
        ...
        $method = $target
            ? __FUNCTION__ . \Includes\Utils\Converter::convertToUpperCamelCase($target)
            : null;
        ...
        if ($data) {
            ...
            if (isset($aliases[$target])) {
                $extLength = strlen('.' . static::CLEAN_URL_DEFAULT_EXTENSION);

                return (substr($aliases[$target], -$extLength) === '.' . static::CLEAN_URL_DEFAULT_EXTENSION)
                    ? $aliases[$target]
                    : $aliases[$target] . '/';
            }
        }
    ...
    public function isUseCanonicalProductURLWithCategory()
    {
        ...
        return $formats['canonical_product'] === static::PRODUCT_URL_CANONICAL_WITH_CATEGORY;
    }
    ...
    protected function buildURLProduct($params)
    {
        ...
        if (!empty($params['product_id'])) {
            ...
            if ($product !== null && $product->getCleanURL()) {
    ...
    protected function buildURLCategory($params)
    {
        ...
        $id = $params['category_id'] ?? ($params['id'] ?? null);
        if ($id) {
    ...
    public function buildCategoryCleanUrl(int $categoryId): string
    {
        [$urlParts, $params] = $this->buildURLCategory(['category_id' => $categoryId]);

        return $this->buildUrlByData('category', $urlParts, $params);
    }
...
//        }
    ...
    public function buildEditURL($entity)
    {
        ...
        $data = method_exists($this, $method)
            ? $this->{$method}($entity)
            : [
                \Includes\Utils\Converter::convertFromCamelCase($entityType),
