...
class LanguageLabel extends \XLite\Model\Repo\Base\I18n
{
    ...
    public const SEARCH_CODES      = 'codes';
    public const ORDER_FIRST_BY_IDS = 'orderFirstByIds';
    ...
    public function findLabelsByCode($code = null, $count = false)
    {
        ...
        $data = $this->getFromCache('all_by_code', ['code' => $code]);
        ...
        if (!isset($data)) {
            ...
            if (
                $this->getTranslationCode() != \XLite::getDefaultLanguage()
                || \XLite::getDefaultLanguage() !== \XLite\Core\Translation::DEFAULT_LANGUAGE
            ) {
            ...
            $this->saveToCache($data, 'all_by_code', ['code' => $code]);
        }
    ...
    protected function prepareCndSubstring(\Doctrine\ORM\QueryBuilder $queryBuilder, $value)
    {
        if (!empty($value)) {
            ...
            $or->add('CAST(l.name AS CHAR) LIKE :substring');
    ...
    {
        $queryBuilder->addSelect('FIND_IN_SET(l.label_id, :label_ids) new_index')
