...
class State extends \XLite\Model\Repo\ARepo
{
    ...
    public const P_COUNTRY_CODE    = 'countryCode';
    ...
    {
        $result = $this->getFromCache('codes', ['state_id' => $stateId]);

        if (!isset($result)) {
            $entity = $this->defineGetCodeByIdQuery($stateId)->getSingleResult();
            $result = $entity ? $entity->getCode() : '';

            $this->saveToCache($result, 'codes', ['state_id' => $stateId]);
        }
    ...
    public function findByCountryCode($countryCode)
    {
        ...
        return $country ? $this->defineByCountryQuery($country)->getResult() : [];
    }
    ...
    protected function defineCacheCells()
    {
        ...
        $list['all'] = [
            static::RELATION_CACHE_CELL => ['\XLite\Model\Country'],
        ];
        ...
        $list['codes'] = [
            static::ATTRS_CACHE_CELL => ['state_id'],
        ];
