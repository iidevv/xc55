...
abstract class Processor extends \XLite\Base
{
    ...
    public const SILENT       = 'W';
    public const SEPARATE     = 'E';
    public const COMPLETED    = 'S';
    ...
    public function isTransactionAllowed(\XLite\Model\Payment\Transaction $transaction, $transactionType)
    {
        ...
        if (in_array($transactionType, $this->getAllowedTransactions())) {
            $methodName = 'is' . \Includes\Utils\Converter::convertToUpperCamelCase($transactionType) . 'TransactionAllowed';
            ...
            if (method_exists($this, $methodName)) {
                $result = $this->{'is' . \Includes\Utils\Converter::convertToUpperCamelCase($transactionType) . 'TransactionAllowed'}($transaction);
            }
        }
    ...
    {
        $method = 'get' . \Includes\Utils\Converter::convertToUpperCamelCase($transactionType) . 'TransactionValue';
    ...
    public function doTransaction(\XLite\Model\Payment\Transaction $transaction, $transactionType)
    {
        if ($this->isTransactionAllowed($transaction, $transactionType)) {
            $methodName = 'do' . \Includes\Utils\Converter::convertToUpperCamelCase($transactionType);
     ...
     * @return string
     */
    public function pay(\XLite\Model\Payment\Transaction $transaction, array $request = [])
    {
        $this->transaction = $transaction;
    ...
    public function getInputDataFields()
    {
        ...
        foreach ($labels as $key => $value) {
            $result[$key] = [
                'label'       => $value,
                'accessLevel' => $accessLevels[$key] ?? \XLite\Model\Payment\TransactionData::ACCESS_ADMIN,
            ];
        }
    ...
    {
        $transaction = $backendTransaction ?? $this->transaction;
    ...
    {
        return $method->getSetting('mode') === \XLite\View\FormField\Select\TestLiveMode::TEST;
    }
