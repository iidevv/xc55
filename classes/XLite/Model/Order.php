...
use XCart\Domain\GmvTrackerDomain;
use XCart\Framework\ApiPlatform\Core\Bridge\Doctrine\Orm\Filter\IntegerDateFilter;
use XLite\API\Endpoint\Order\DTO\OrderOutput as Output;
 ...
 *          @ORM\Index (name="total", columns={"total"}),
 *          @ORM\Index (name="subtotal", columns={"subtotal"}),
 *          @ORM\Index (name="tracking", columns={"tracking"}),
 *          @ORM\Index (name="payment_status", columns={"payment_status_id"}),
 *          @ORM\Index (name="shipping_status", columns={"shipping_status_id"}),
 *          @ORM\Index (name="shipping_id", columns={"shipping_id"}),
 *          @ORM\Index (name="lastRenewDate", columns={"lastRenewDate"}),
 *          @ORM\Index (name="orderNumber", columns={"orderNumber"}),
 *          @ORM\Index (name="is_order", columns={"is_order"}),
 *          @ORM\Index (name="xcPendingExport", columns={"xcPendingExport"})
 *      }
 * )
 *
 ...
 * @ORM\DiscriminatorColumn   (name="is_order", type="integer", length=1)
 * @ORM\DiscriminatorMap      ({1 = "XLite\Model\Order", 0 = "XLite\Model\Cart"})
 ...
 *          "get_by_number"={
 *              "method"="GET",
 *              "path"="/orders/by_number/{orderNumber}.{_format}",
 ...
 *     }
 * )
 * @ApiPlatform\ApiFilter(IntegerDateFilter::class, properties={"date"})
 * @ApiPlatform\ApiFilter(OrderFilter::class, properties={"date","lastRenewDate"}, arguments={"orderParameterName"="order"})
 * @ApiPlatform\ApiFilter(SearchFilter::class, properties={"paymentStatus.code"="exact", "shippingStatus.code"="exact", "profile.login"="partial"})
 */
class Order extends \XLite\Model\Base\SurchargeOwner
{
     ...
     */
    public const ORDER_ZERO = 0.00001;

    /**
     * Add item error codes
     */
    public const NOT_VALID_ERROR = 'notValid';
     ...
     * @var \XLite\Model\Profile
     *
     * @ORM\ManyToOne  (targetEntity="XLite\Model\Profile", cascade={"merge","detach","persist"})
     * @ORM\JoinColumn (name="orig_profile_id", referencedColumnName="profile_id", onDelete="SET NULL")
     */
     ...
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\OrderDetail", mappedBy="order", cascade={"all"})
     ...
     * @var \Doctrine\Common\Collections\Collection
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\OrderTrackingNumber", mappedBy="order", cascade={"all"})
     */
     ...
     * @var \Doctrine\Common\Collections\Collection
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\OrderHistoryEvents", mappedBy="order", cascade={"all"})
     */
    protected $events;
     ...
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\OrderItem", mappedBy="order", cascade={"all"})
     */
    protected $items;
     ...
     * @var \Doctrine\Common\Collections\Collection
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\Order\Surcharge", mappedBy="owner", cascade={"all"})
     ...
     * @var \Doctrine\Common\Collections\Collection
     *
     * @ORM\OneToMany (targetEntity="XLite\Model\Payment\Transaction", mappedBy="order", cascade={"all"})
     */
     ...
     *
     * @ORM\ManyToOne  (targetEntity="XLite\Model\Currency", inversedBy="orders", cascade={"merge","detach"})
     ...
     * @var ArrayCollection
     *
     * @ORM\ManyToMany (targetEntity="XLite\Model\Order")
     * @ORM\JoinTable  (
     *      name="order_backorder_competitors",
     *      joinColumns={@ORM\JoinColumn(name="id", referencedColumnName="order_id", onDelete="CASCADE")},
     *      inverseJoinColumns={@ORM\JoinColumn(name="competitor_id", referencedColumnName="order_id", onDelete="CASCADE")}
     * )
     */
    ...
    public function addItem(\XLite\Model\OrderItem $newItem)
    {
        ...
        if ($newItem->isValid() && $newItem->isConfigured()) {
            ...
            if ($item) {
                ...
                $item->refreshUpdateDate();
            } else {
                $newItem->refreshUpdateDate();
     ...
     */
    public function removeItem(\XLite\Model\OrderItem $item)
    {
        if ($item->getAmount() > 1) {
    ...
    public function getSourceAddress()
    {
        if ($this->sourceAddress === null) {
            ...
            $config = $this->getSourceAddressConfiguration();
            ...
            $address->setPhone($config->company_phone);
    ...
    /**
     * Returns company configuration for getSourceAddress()
     *
     * @return \XLite\Core\ConfigCell
     */
    protected function getSourceAddressConfiguration()
    {
    ...
    /**
     * Send order awaiting approval notification if needed
     */
    ...
    public function getPaymentMethods()
    {
        ...
        if (\XLite\Core\Auth::getInstance()->isOperatingAsUserMode()) {
            ...
            $filteredFakeMethods = array_filter(
                $fakeMethods,
                static function ($fakeMethod) use ($list) {
                    $fakeServiceName = $fakeMethod->getServiceName();

                    // Check if $list already contains fake method
                    return !array_reduce($list, static function ($carry, $method) use ($fakeServiceName) {
    ...
    {
        return $this->getShippingStatus() === null;
    }
    ...
    public function getPaymentMethod()
    {
        ...
        if (!$transaction) {
            $transaction = $this->hasUnpaidTotal() || count($this->getPaymentTransactions()) === 0
    ...
    public function setPaymentMethod($paymentMethod, $value = null)
    {
        ...
        if ($paymentMethod === null || $this->getFirstOpenPaymentTransaction()) {
            $transaction = $this->getFirstOpenPaymentTransaction();
            if ($transaction) {
    ...
    public function getVisiblePaymentMethods()
    {
        ...
        if (count($result) === 0 && 0 < count($this->getPaymentTransactions())) {
    ...
    public function getFirstOpenPaymentTransaction()
    {
        ...
        return \Includes\Utils\ArrayManager::findValue(
            $transactions,
            [$this, 'checkPaymentTransactionOpen']
        );
    }
    ...
    {
        if (!\XLite\Model\Shipping::getInstance()->shouldAllowLongCalculations()) {
            return;
        }
    ...
    public function getShippingProcessor()
    {
        if ($this->shippingProcessor === null) {
            $shipping = \XLite\Core\Database::getRepo('XLite\Model\Shipping\Method')->find($this->getShippingId());
            $this->shippingProcessor = $shipping && $shipping->getProcessorObject()
                ? $shipping->getProcessorObject()
                : false;
        }
    ...
    public function setIsNotificationsAllowedFlag($value)
    {
        ...
        $this->isNotificationsAllowedFlag = (bool)$value;
    ...
    public function setIgnoreCustomerNotifications($value)
    {
        ...
        $this->ignoreCustomerNotifications = (bool)$value;
    ...
    public function getSurchargesSubtotal($type = null, $include = null)
    {
        ...
        foreach ($surcharges as $surcharge) {
            if ($surcharge->getAvailable() && ($include === null || $surcharge->getInclude() === $include)) {
        ...
        if ($this->statusIsSet) {
            ...
            foreach (['payment', 'shipping'] as $type) {
                ...
                if (
                    ...
                    $em->addAfterFlushCallback(function () use ($oldStatus, $status, $type) {
                        ...
                        $this->processStatusAnyToAny($oldStatus, $status, $type);
                    });
            ...
            $em->addAfterFlushCallback(function () use ($changed, $statusHandlers) {
                if ($changed) {
                    $this->checkInventory();
                }
    ...
    /**
     * These handlers will be used in systemStatus1->...customStatus(es)..->systemStatus2 transition
     * by default, don't try to cast old statuses
     * @param string $type Type
     *
     * @return array
     */
    ...
    protected function getStatusHandlers($oldStatus, $newStatus, $type)
    {
        ...
        if ($oldCode && $newCode && isset($statusHandlers[$oldCode][$newCode])) {
    ...
    /**
     * @param array $inStatusHandlers
     *
     * @return array
     */
    protected function getFlatStatuses($inStatusHandlers = [])
    {
        $flatStatuses = array_reduce($inStatusHandlers, static function ($a, $b) {
            return array_merge($a, (array) $b);
        }, []);

        return array_unique(array_merge(array_keys($flatStatuses), array_keys($inStatusHandlers)));
    }
    ...
    {
        /** @var GmvTrackerDomain $gmvTracker */
        $gmvTracker = \XCart\Container::getContainer()->get(GmvTrackerDomain::class);
        $gmvOrderData = $gmvTracker->prepareOrderGmvData($this);
        $gmvTracker->saveOrderGmvData($gmvOrderData);
    ...
    /**
     * Status change handler for the "any to any" event.
     * 1)Allows to run handlers on systemStatus1->...customStatus(es)..->systemStatus2 transition
     *   Attention! handlers must include internal checking to avoid double execution
     *
     * @param $oldStatus
     * @param $newStatus
     * @param $type
     */
    protected function processStatusAnyToAny($oldStatus, $newStatus, $type)
    {
        // here is the key point to decide if we try to find an old system status in order history
        $statusHandlers2Cast = $this->getStatusHandlersForCast($type);

        if (!$statusHandlers2Cast) {
            return;
        }
        // Transitions between these statuses are valued only
        $businessLogicStatuses = $this->getFlatStatuses($statusHandlers2Cast);

        $inOldCode = $oldStatus->getCode();
        $inNewCode = $newStatus->getCode();

        // cycle is used for performance only, 'continue' below avoids unnecessary code execution
        foreach ($statusHandlers2Cast as $oldCode2Deal => $newCodes2Deal) {
            if (
                $oldCode2Deal === $inOldCode // exact handler will be returned/launched by getStatusHandlers, do nothing
                || !in_array($inNewCode, array_keys($newCodes2Deal)) // nothing to deal with here
                || in_array($inOldCode, $businessLogicStatuses) // take into account transition from custom statuses to business statuses only
            ) {
                continue;
            }

            // find the nearest business(system) status code from order history
            $oldSystemBusinessCode = $this->getNearestOldBusinessCode($inOldCode, $businessLogicStatuses, $type);

            if (
                $oldSystemBusinessCode !== $inOldCode
                && isset($statusHandlers2Cast[$oldSystemBusinessCode][$inNewCode])
            ) {
                // We have found an old business code before a bunch of meaningless ones
                $castHandlers = is_array($statusHandlers2Cast[$oldSystemBusinessCode][$inNewCode])
                    ? array_unique($statusHandlers2Cast[$oldSystemBusinessCode][$inNewCode])
                    : [$statusHandlers2Cast[$oldSystemBusinessCode][$inNewCode]];

                foreach ($castHandlers as $castHandler) {
                    // here we run a handler for a cast oldBusinessStatus->...someStatuses...->newBusinessStatus transition
                    $methodOnStatusChange = 'process' . \Includes\Utils\Converter::convertToUpperCamelCase($castHandler);
                    if (method_exists($this, $methodOnStatusChange)) {
                        $this->$methodOnStatusChange();
                    }
                }
                // typecast old code only once
                break;
            }
        }
    }
    ...
    /**
     * Find backwards the nearest old business(system) code before a bunch of non-business logic codes
     *
     * @param $meaninglessOldCode
     * @param $businessLogicStatuses
     * @param $type
     *
     * @return string
     */
    protected function getNearestOldBusinessCode($meaninglessOldCode, $businessLogicStatuses, $type)
    {
        // this shared static cache is supposed to be used in other functions in the future. use the key ['OrderHistoryEvents', $this->getOrderId()]
        $historyList = $this->executeCachedRuntime(function () {
            return \XLite\Core\Database::getRepo('XLite\Model\OrderHistoryEvents')->findAllByOrder($this) ?: [];
        }, ['OrderHistoryEvents', $this->getOrderId()]) ?: [];

        $eventCodeName = $type === 'payment' ? \XLite\Core\OrderHistory::CODE_CHANGE_PAYMENT_STATUS_ORDER : \XLite\Core\OrderHistory::CODE_CHANGE_SHIPPING_STATUS_ORDER;
        $historyList = array_filter($historyList, static function ($event) use ($eventCodeName) {
            return $event->getCode() === $eventCodeName;
        });

        foreach ($historyList as $event) {
            $oldOrderStatus = $event->getData()['oldStatusCode'];
            if (in_array($oldOrderStatus, $businessLogicStatuses)) {
                return $oldOrderStatus;
            }
        }

        return $meaninglessOldCode;
    }
    // }}}
    ...
    protected function changeItemsInventory($sign)
    {
        ...
        foreach ($this->getItems() as $item) {
            $this->changeItemInventory($item, $sign, !$registerAsGroup);
        }
    }
    ...
    {
        $delta = $sign * $item->getAmount();

        $realDelta = $delta > 0 && $item->getBackorderedAmount() > 0
            ? $delta - $item->getBackorderedAmount()
            : $delta;

        if ($realDelta !== 0) {
            if ($register) {
                $this->registerHistoryChangeItemAmount($item, $realDelta);
            }
            $item->changeAmount($realDelta);
        }
     ...
     * @param integer                $delta
     */
    protected function registerHistoryChangeItemAmount($item, $delta)
    {
        $history = \XLite\Core\OrderHistory::getInstance();
        $history->registerChangeAmount($this->getOrderId(), $item->getProduct(), $delta);
    }
    ...
    {
        if ($this->paymentTransactionSums === null || $override) {
            $transactions = $this->getPaymentTransactions();
    ...
    public function getCalculatedPaymentStatus($override = false)
    {
        ...
        if ($result === \XLite\Model\Order\Status\Payment::STATUS_QUEUED) {
            if ($lastTransaction) {
    ...
    protected function getSalesDelta()
    {
        ...
        if ($newStatusCode) {
            ...
            if (
                (!$oldStatusCode || !in_array($oldStatusCode, $paidStatuses, true))
     ...
     * @return Order
     */
    public function addItems(\XLite\Model\OrderItem $item)
    {
        $this->items[] = $item;
        return $this;
    }
