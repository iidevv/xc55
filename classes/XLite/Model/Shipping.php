...
class Shipping extends \XLite\Base\Singleton
{
     ...
     * @var array
     */
    public static $runtimeDefaultAddressCache = false;
     ...
     */
    protected static $registeredProcessors = [];
    ...
    {
        if (
            !isset(static::$registeredProcessors[$processorClass])
            && class_exists($processorClass)
        ) {
            static::$registeredProcessors[$processorClass] = new $processorClass();
            uasort(static::$registeredProcessors, [\XLite\Model\Shipping::getInstance(), 'compareProcessors']);
        }
    }
    ...
    public function shouldAllowLongCalculations()
    {
        ...
        return (\XLite::getController()->isAJAX() && $isValidAjaxRequests)
            || (!\XLite::getController()->isAJAX() && \XLite::getController()->getTarget() === 'checkout')
            || \XLite::isAdminZone();
    }
    ...
    public function getDestinationAddress(\XLite\Logic\Order\Modifier\Shipping $modifier)
    {
        ...
        return $address ?? static::getDefaultAddress();
    }
    ...
    public static function getDefaultAddress()
    {
        if (static::$runtimeDefaultAddressCache === false) {
            ...
            static::$runtimeDefaultAddressCache = [
    ...
    protected function getProcessorRates($processor, $modifier)
    {
        ...
        return $rates && is_array($rates) ? $rates : [];
    }
    ...
    {
        $savedRatesValues = \XLite\Core\Session::getInstance()->savedRatesValues ?: [];
        $hash = [];
