...
class Info extends \XLite\Model\DTO\Base\ADTO
{
    ...
    public static function validate($dto, ExecutionContextInterface $context)
    {
        if (!empty($dto->default->sku) && !static::isSKUValid($dto)) {
            static::addViolation($context, 'default.sku', Translation::lbl('This product SKU already exists'));
        }
    ...
    protected static function isSKUValid($dto)
    {
        ...
        $entitiesWithSameSku = \XLite\Core\Database::getRepo('XLite\Model\Product')->findBySku($sku);

        return $entitiesWithSameSku
            ? count($entitiesWithSameSku) === 1 && (int) $entitiesWithSameSku[0]->getProductId() === (int) $identity
            : true;
    }
    ...
    protected function init($object)
    {
        ...
        $shipping       = [
            'requires_shipping_status' => $object->getShippable(),
        ...
        $cleanURL = new CommonCell([
            'autogenerate' => !(bool) $object->getCleanURL(),
    ...
    public function populateTo($object, $rawData = null)
    {
        ...
        $object->setEnabled((bool) $default->available_for_sale);
        ...
        $object->setInventoryEnabled((bool) $priceAndInventory->inventory_tracking->inventory_tracking);
        ...
        $object->setShippable((bool) $shipping->requires_shipping_status);

        $shippingBox = $shipping->requires_shipping->shipping_box;
        $object->setUseSeparateBox((bool) $shippingBox->separate_box);

        $object->setBoxLength($shipping->dimensions['length']);
        $object->setBoxWidth($shipping->dimensions['width']);
        $object->setBoxHeight($shipping->dimensions['height']);
