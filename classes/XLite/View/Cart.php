...
class Cart extends \XLite\View\Dialog
{
    ...
    protected function canEstimateShipping(): bool
    {
        /** @var Method $method */
        foreach (Database::getRepo('XLite\Model\Shipping\Method')->findAsOptions() as $method) {
            if ($method->canBeEstimated()) {
                return true;
            }
        }
    ...
    protected function isProposeShippingEstimation(): bool
    {
        return !in_array('SHIPPING', array_keys($this->getSurchargeTotals()), true)
            && $this->getCart()->isShippable()
            && $this->canEstimateShipping();
    }
}
