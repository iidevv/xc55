...
class Info extends \XLite\View\AView implements ProviderInterface
{
    ...
    public function getCSSFiles()
    {
        ...
        $list[] = 'address/order/style.less';
        $list[] = 'select_address/style.less';
    ...
    protected function isHistoryVisible()
    {
        ...
        if ($this->getOrder()) {
            $result = (bool) \XLite\Core\Database::getRepo('XLite\Model\OrderHistoryEvents')->findOneBy(['order' => $this->getOrder()]);
        }
    ...
    protected function getBackorderWarningText()
    {
        if ($backorderCompetitiorsText = $this->getBackorderCompetitorsText()) {
            $warningText = static::t('This order was placed at the same time as {{orders}}; as a result, some items were out of stock at the time of order placement.', [
                'orders' => $backorderCompetitiorsText,
                'kb_link' => 'https://support.x-cart.com/en/articles/4634599-order-payment-statuses-in-x-cart',
            ]);
        } else {
