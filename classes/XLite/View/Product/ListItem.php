...
class ListItem extends \XLite\View\AView implements DynamicWidgetInterface
{
    ...
    public const PARAM_VIEW_LIST_NAME                    = 'productViewListName';
    public const PARAM_DISPLAY_MODE                      = 'displayMode';
    public const PARAM_ITEM_LIST_WIDGET_TARGET           = 'itemListWidgetTarget';
    ...
    public const PARAM_PRODUCT_ENTITY_VERSION_FETCHER    = 'productEntityVersionFetcher';
    ...
    public function getDynamicProductCellClasses()
    {
        $classes = [
            ...
            $this->getIsAllStockInCartCellClass(),
        ];
    ...
    {
        $widget = $this->getChildWidget(ProductAddedToCartCellClass::class, [
    ...
    /**
     * Get content of the dynamic widget that renders 'all-stock-in-cart' css class if all stock was added to cart.
     *
     * @return string
     */
    public function getIsAllStockInCartCellClass()
    {
        $widget = $this->getChildWidget(IsAllStockInCartCellClass::class, [
            IsAllStockInCartCellClass::PARAM_PRODUCT_STOCK_AVAILABILITY_POLICY => $this->getParam(self::PARAM_PRODUCT_STOCK_AVAILABILITY_POLICY),
        ]);
    ...
    {
        return \XLite\Core\Config::getInstance()->General->enable_add2cart_button_grid;
    }
    ...
    protected function getAdd2CartBlockWidget()
    {
        ...
        if ($product->isOutOfStock()) {
            ...
            $classes = 'add-to-cart product-add2cart productid-' . $product->getProductId();
     ...
     * @return \XLite\View\AView
     */
    protected function getChangeQtyBlockWidget()
    {
        return $this->getChildWidget(
            ChangeQty::class,
            [
                ChangeQty::PARAM_PRODUCT_ID                        => $this->getProduct()->getProductId(),
                ChangeQty::PARAM_PRODUCT_STOCK_AVAILABILITY_POLICY => $this->getParam(self::PARAM_PRODUCT_STOCK_AVAILABILITY_POLICY),
                ChangeQty::PARAM_ALL_STOCK_IN_CART_TOOLTIP_TEXT    => $this->getProduct()->getAllStockInCartTooltipText(),
            ]
        );
    }
