...
class Attributes extends \XLite\View\Product\Details\AAttributes
{
    ...
    protected function isAttributeVisible($attr, $value)
    {
        ...
        if (
            $attr::TYPE_CHECKBOX === $attr->getType()
    ...
    protected function postprocessAttributes(array $attributes)
    {
        ...
        $visibleAttributeIds = array_map(static function ($attribute) {
            return $attribute->getId();
        }, $product->getVisibleAttributes());

        if ($product && !\XLite::isAdminZone()) {
            foreach ($attributes as $i => $attribute) {
                if (!in_array($attribute->getId(), $visibleAttributeIds)) {
                    unset($attributes[$i]);
                    continue;
                }

                if (
                    $attribute->getType() === \XLite\Model\Attribute::TYPE_TEXT
