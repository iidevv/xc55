...
class SelectedDecider implements \Serializable
{
    ...
    protected function findAndSetSelectedItem($target, $items = null)
    {
        ...
        foreach ($items as $index => $item) {
            if (
                isset($item[AAdmin::ITEM_CHILDREN])
            ...
            if (isset($item[AAdmin::ITEM_TARGET])) {
                ...
                if ($selected && !empty($item[AAdmin::ITEM_PERMISSION])) {
                    if (is_array($item[AAdmin::ITEM_PERMISSION])) {
                        $hasPermission = false;
                        foreach ($item[AAdmin::ITEM_PERMISSION] as $permission) {
                        ...
                        $selected = $auth->isPermissionAllowed($item[AAdmin::ITEM_PERMISSION]);
                    }
                }
            ...
            foreach ($items as $index => $item) {
                ...
                if (isset($item[AAdmin::ITEM_CHILDREN]) && $item[AAdmin::ITEM_CHILDREN]) {
                    ...
                    foreach ($children as $child) {
                        if (
                            (isset($child['decider_selected']) && $child['decider_selected'])
    ...
    {
        [$this->menuClassName, $this->getter] = unserialize($serialized);
    }
}
