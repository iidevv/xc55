...
class FileUploader extends \XLite\View\AView
{
    ...
    public const PARAM_HELP_MESSAGE       = 'helpMessage';
    public const PARAM_MAX_WIDTH          = 'maxWidth';
    public const PARAM_MAX_HEIGHT         = 'maxHeight';
    public const PARAM_IS_IMAGE           = 'isImage';
    public const PARAM_IS_TEMPORARY       = 'isTemporary';
    public const PARAM_NAME               = 'fieldName';
    public const PARAM_MULTIPLE           = 'multiple';
    public const PARAM_POSITION           = 'position';
    public const PARAM_IS_VIA_URL_ALLOWED = 'isViaUrlAllowed';
    public const PARAM_IS_REMOVABLE       = 'removable';
    public const PARAM_HAS_ALT            = 'hasAlt';
    ...
    protected function getName()
    {
        ...
        if ($this->getParam(static::PARAM_MULTIPLE)) {
            ...
            if (!$index) {
                if ($this->getObject()) {
                    $index = (int)$this->getObject()->getId();
    ...
    protected function getVModel()
    {
        ...
        foreach ($parts as $part) {
            ...
            if (is_numeric($part) && (int)$part == $part) {
                $name .= '[' . $part . ']';
            } else {
                $name .= '.' . $part;
            }
    ...
    {
        $result = $this->getParam(static::PARAM_IS_IMAGE)
    ...
    protected function getOptions(): array
    {
        return [
            ...
            'defaultErrorMessage' => static::t($this->getErrorMessageDefault()),
            'alt'                 => $this->hasAlt() ? $this->getAlt() : '',
            'initialAlt'          => $this->hasAlt() ? $this->getAlt() : '',
            'realErrorMessage'    => static::t($this->getMessage()),
            'preview'             => $this->getPreview(),
            'link'                => $this->getLink(),
            'temp_id'             => $this->isTemporary() ? $this->getObject()->getId() : null,
        ];
    }
