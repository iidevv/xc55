...
class FormGenerator
{
            ...
            if (array_key_exists('constraints', $options)) {
                ...
                foreach ($options['constraints'] as $constraint => $constraintOptions) {
                    ...
                    $validators[$validatorConstraint] = [
                        ...
                        'message' => $constraintOptions['message'] ?? 'NotValid'
                    ];
                    ...
                    if (
                        ($constraint === 'XLite\Core\Validator\Constraints\MaxLength')
    ...
    public function generate($schema, $dataObject = null, $viewObject = null, array $options = [])
    {
        ...
        $options['data_object'] = $dataObject
            ? json_encode($dataObject->toArray(), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE)
            : '';
    ...
    {
        $name = [];
        $moduleId = Module::getModuleIdByClassName($constraint);

        if ($moduleId && $moduleId !== 'Symfony-Component') {
            $name = explode(Module::ID_SEPARATOR, $moduleId);
        }
