...
class FormTypeExtension extends AbstractTypeExtension
{
    ...
    public function buildView(FormView $view, FormInterface $form, array $options)
    {
        ...
        $vModel = !in_array($view->vars['full_name'], ['form[returnURL]', 'form[' . \XLite::FORM_ID . ']'], true)
            ? str_replace(['[]', '[', ']'], ['', '.', ''], $view->vars['full_name'])
            : '';
        $vValidatePath = explode('.', $vModel);
        $validationRoot = array_shift($vValidatePath);
        $vValidatePath = implode('_', array_map(['\Includes\Utils\Converter', 'convertToUpperCamelCase'], $vValidatePath));
        ...
        $rowClasses = implode(' ', array_map(static function ($item) {
