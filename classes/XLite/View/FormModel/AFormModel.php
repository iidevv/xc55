...
abstract class AFormModel extends \XLite\View\AView
{
    ...
    protected static function correctSchemaDependency($schema, $section, $field)
    {
        foreach ($schema as $sectionName => $sectionDefinition) {
            foreach ($sectionDefinition as $fieldName => $fieldDefinition) {
                ...
                if (
                    isset($fieldDefinition['type'], $fieldDefinition['fields'])
    ...
    protected static function correctFieldDependency($fieldDefinition, $dependencyType, $section, $field)
    {
         ...
         * @return boolean
         */
        $isEndRule = static function ($dependencyRule) {
    ...
    public function getJSFiles()
    {
        ...
        return array_merge(
            ...
            $this->jsFiles ? call_user_func_array('array_merge', array_values($this->jsFiles)) : []
        );
    }
    ...
    public function getCSSFiles()
    {
        ...
        return array_merge(
            parent::getCSSFiles(),
            $this->cssFiles ? call_user_func_array('array_merge', array_values($this->cssFiles)) : []
        );
    }
    ...
    public function getForm()
    {
        ...
        $dataObject = $this->getInitDataObject() ?? $this->getDataObject();
    ...
    protected function getCommonFiles()
    {
        ...
        $list[static::RESOURCE_JS][] = 'vue/vuelidate.min.js';
        $list[static::RESOURCE_JS][] = 'vue/validators.min.js';
        ...
        $subResources = $this->commonFiles
            ? call_user_func_array('array_merge_recursive', array_values($this->commonFiles))
            : [];
    ...
    protected function getInitDataObject()
    {
        $controller = \XLite::getController();
        $getter = 'getFormModelTmpData';

        $data = method_exists($controller, $getter) ? $controller->{$getter}() : null;
        if ($data) {
            $generator = new FormGenerator();
            $dataObject = $this->getDataObject();
            $viewObject = $this->getViewObject() ?: $dataObject;

            $form = $generator->generate($this->getSchema(), $dataObject, $viewObject, $this->getFormOptions($dataObject));
            $form->submit($data);
            return $form->getData();
        }
    ...
    protected function getFormAction()
    {
        ...
        return $target !== null ? \XLite\Core\Converter::buildURL($target, $action, $params) : '';
    }
    ...
    protected function getReturnURL()
    {
        ...
        return $target !== null ? \XLite\Core\Converter::buildURL($target, '', $params) : '';
    }
    ...
    {
        return array_values(array_filter(array_unique($this->theme)));
    }
    ...
    protected function getListName()
    {
        ...
        if (preg_match('/\\\([a-z0-9]+)\\\([a-z0-9]+)\\\View\\\FormModel\\\/Si', get_called_class(), $match)) {
