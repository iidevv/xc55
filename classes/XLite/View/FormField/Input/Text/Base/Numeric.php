...
abstract class Numeric extends \XLite\View\FormField\Input\Text
{
    ...
    public const PARAM_MOUSE_WHEEL_ICON = 'mouseWheelIcon';
    public const PARAM_ALLOW_EMPTY      = 'allowEmpty';
    ...
    protected function checkMinValue()
    {
        ...
        if (!is_null($min) && (float) $this->getValue() < $min) {
            $result = false;
    ...
    protected function checkMaxValue()
    {
        ...
        if (!is_null($max) && (float) $this->getValue() > $max) {
            $result = false;
            $this->errorMessage = \XLite\Core\Translation::lbl(
