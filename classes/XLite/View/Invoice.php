...
class Invoice extends \XLite\View\AView
{
    ...
    {
        return static::t('Invoice X', ['id' => $this->getOrder()->getOrderNumber()]);
    }
    ...
    protected function formatSurcharge(array $surcharge)
    {
        return static::formatPrice(
            ...
            \XLite\Core\Layout::getInstance()->getZone() !== \XLite::ZONE_ADMIN
        );
    }
    ...
    {
        $method = 'getAddressField' . \Includes\Utils\Converter::convertToUpperCamelCase($serviceName) . 'Template';

        return method_exists($this, $method)
            ? call_user_func([$this, $method], $type, $field)
    ...
    {
        return $type === \XLite\Model\Address::SHIPPING
