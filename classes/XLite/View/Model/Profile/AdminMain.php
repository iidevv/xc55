...
class AdminMain extends \XLite\View\Model\AModel
{
    ...
    {
        return ($this->currentAction === 'validateInput') ?: parent::isValid();
    }
    ...
    public function getDefaultFieldValue($name)
    {
        ...
        switch ($name) {
            ...
            case 'roles':
                ...
                if ($this->getModelObject()->getProfileId()) {
                    break;
                }
    ...
    {
        return parent::isShowSectionHeader($section) && $section != self::SECTION_SUMMARY;
    }
    ...
    protected function getFormFieldsForSectionAccess()
    {
        if (
            ...
            && (int) Auth::getInstance()->getProfile()->getProfileId() === (int) $this->getModelObject()->getProfileId()
        ) {
            return null;
        }
        ...
        $roles = \XLite\Model\Repo\Role::getAvailableRoles();
        ...
        if (
            $this->getModelObject()
            && $auth->hasRootAccess()
            && $auth->getProfile()->getProfileId() === $this->getModelObject()->getProfileId()
        ) {
    ...
    protected function getModelObjectValue($name)
    {
        ...
        return $value ?? parent::getModelObjectValue($name);
    }
    ...
    protected function isAllowedToEditAdminProfiles(): bool
    {
        return Auth::getInstance()->isPermissionAllowed(\XLite\Model\Role\Permission::ROOT_ACCESS);
    }
    ...
    protected function setModelProperties(array $data)
    {
        ...
        if (
            ...
            // WA for XCB-344. Block incoming values like en_US xlite_profiles.language is varchar(2)
            unset($data['language']);
        }
        ...
        $isAllowedToEditAdminProfiles = $this->isAllowedToEditAdminProfiles();

        $profileId = $this->getModelObject()->getProfileId();
        $unavailableRoles = array_filter(
            Database::getRepo('XLite\Model\Role')->findBy([
                'id' => array_map(
                    static fn(Role $role): int => $role->getId(),
                    \XLite\Model\Repo\Role::getUnavailableRoles()
                )
            ]),
            static fn(Role $role): bool => in_array(
                $profileId,
                array_map(
                    static fn(Profile $profile): int => $profile->getProfileId(),
                    $role->getProfiles()->toArray()
                )
            )
        );
        ...
        if ($isAdmin && !isset($data['roles']) && $isAllowedToEditAdminProfiles && !$this->isLoggedProfile()) {
            $data['roles'] = [];
        }
        ...
        foreach ($unavailableRoles as $role) {
            $role->addProfiles($model);
        }
    ...
    protected function needSetRootAccess(\XLite\Model\Profile $profile)
    {
        ...
        return \XLite\Core\Database::getRepo('XLite\Model\Role')->count() == 1
               || $onlyOneRootAdmin;
    }
     ...
     * @return string
     */
    protected function prepareErrorMessage($message, array $data): string
    {
        return $this->showErrorsViaTopMessage() ? $message : parent::prepareErrorMessage($message, $data);
    }
        ...
        if (
            ...
            if ($data['password'] !== $data['password_conf']) {
                ...
                $this->addErrorMessage(
                    'password',
                    static::t('Password and its confirmation do not match'),
     ...
     * @return bool
     */
    protected function validateFormFieldPasswordValue($field, $formFields)
    {
        $result = true;
        ...
        if (
            $data && $model
            && !empty($data['password'])
            && (\strlen($data['password']) < 8 || \strlen($data['password']) > 64)
        ) {
            $result = false;
            $this->addErrorMessage(
                'password',
                static::t('Password must be between 8 and 64 characters long'),
                $formFields[self::SECTION_MAIN]
            );
        }
    ...
    protected function validateFormFieldPasswordCurrentValue($field, $formFields)
    {
        ...
        if (
            ...
            if (
                ($data['login'] !== $savedData['login'] || !empty($data['password']))
    ...
    protected function checkProfileData()
    {
        ...
        if ($result) {
            ...
            if (isset($sameProfile)) {
                ...
                $this->addErrorMessage(
                    'login',
                    static::t('User with specified email is already registered'),
                    $formFields[self::SECTION_MAIN]
                );
                $result = false;
            }
    ...
    protected function performActionUpdate()
    {
        ...
        if ($result && $profile) {
            if (
                isset($data['login'])
                && $profile->getOldLogin()
                && $data['login'] !== $profile->getOldLogin()
                && $profile->getOrdersCount()
            ) {
                Database::getRepo('XLite\Model\Profile')
                    ->updateOrderProfileEmailByOrigProfile($profile, $data['login'], $profile->getOldLogin());
            }
        }
