...
class Main extends \XLite\View\Model\Profile\AProfile
{
    ...
    {
        return self::getRegisterMode() === Request::getInstance()->mode;
    }
    ...
    {
        return $this->checkPassword() && parent::performActionUpdate();
    }
    ...
    protected function prepareDataForMapping()
    {
        ...
        if (isset($data['pending_membership_id']) && 0 < (int) ($data['pending_membership_id'])) {
            $membership = Database::getRepo('XLite\Model\Membership')->find($data['pending_membership_id']);
    ...
    {
        return Request::getInstance()->isAJAX() && $this->isRegisterMode()
            ? false
            : parent::isShowSectionHeader($section);
    }
}
