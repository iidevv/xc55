...
abstract class AItemsList extends \XLite\View\Container
{
    ...
    public const PARAM_AJAX_ITEMS_PER_PAGE = 'itemsPerPage';
     ...
     * @var array
     */
    protected static $sortWidgetIds = [];
    ...
    public static function getSearchValuesStorage($forceFallback = false)
    {
        ...
        if (
            count($searchParamsNamesIntersect) === 0
            || (isset($requestData['mode']) && $requestData['mode'] === 'append')
            || $forceFallback
        ) {
    ...
    {
        return ($sortOrder ?: $this->getSortOrder()) === static::SORT_ORDER_ASC;
    }
