...
class Languages extends \XLite\View\ItemsList\Model\Table
{
    ...
    protected function getColumnClass(array $column, \XLite\Model\AEntity $entity = null)
    {
        ...
        if (
            in_array($column[static::COLUMN_CODE], ['defaultCustomer', 'defaultAdmin'], true)
            && !$entity->getEnabled()
        ) {
            $class .= ' disabled';
        }
    ...
    {
        return $entity->getCode() !== 'en' && !$entity->getValidModule();
    }
    ...
    protected function getLanguageHelpMessage(\XLite\Model\Language $entity)
    {
        ...
        if ($entity->getValidModule()) {
            ...
            $message = static::t('This language is added by module and cannot be removed.', ['module' => $moduleName]);
        } elseif ($entity->getCode() === 'en') {
    ...
    {
        return static::t('X languages have been removed', ['count' => $count]);
    }
