...
class OrderModify extends \XLite\View\AView
{
    ...
    {
        $attributes = [
            'class' => ['address-box', 'billing', 'clearfix'],
        ];

        if (\XLite\Core\Request::getInstance()->type == 'shippingAddress') {
    ...
    {
        $attributes = [
            'class' => ['address-box', 'shipping', 'clearfix'],
        ];

        if (\XLite\Core\Request::getInstance()->type == 'billingAddress') {
            $attributes['class'][] = 'collapsed';
        }
