...
class Translation extends \XLite\Base\Singleton implements \XLite\Base\IREST
{
     ...
     * @return string
     */
    public static function lbl($name, array $arguments = [], $code = null, $type = null)
    {
    ...
    protected function defineDriver()
    {
        ...
        $translationDriver = \Includes\Utils\ConfigParser::getOptions(['other', 'translation_driver']);
        if (defined('LC_CACHE_BUILDING') && !defined('LC_CACHE_BUILDING_FINISH')) {
            $translationDriver = 'DbDirect';
        }

        if ($translationDriver && $translationDriver != 'auto') {
            $class = '\XLite\Core\TranslationDriver\\'
                . \Includes\Utils\Converter::convertToUpperCamelCase($translationDriver);
        ...
        if (!$driver) {
            ...
            if (!isset($driver)) {
                throw new FatalException('Unable to find a translation driver!');
            }
        }
    ...
    public static function formatTimePeriod($seconds, $code = null)
    {
        ...
        if (0 < $h) {
            $result[] = static::lbl('X hours', ['hours' => $h], $code);
        }

        if (0 < $m) {
            $result[] = static::lbl('X minutes', ['minutes' => $m], $code);
        }

        if (0 < $s) {
            $result[] = static::lbl('X seconds', ['seconds' => $s], $code);
        }
