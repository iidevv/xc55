...
class Auth extends \XLite\Base
{
     ...
     * Result codes
     */
    public const RESULT_USER_EXISTS          = 1;
    public const RESULT_REGISTER_SUCCESS     = 2;
    public const RESULT_ACCESS_DENIED        = 3;
    public const RESULT_LAST_ADMIN_ACCOUNT   = 4;
    public const RESULT_LOGIN_IS_LOCKED      = 5;
    public const RESULT_PROFILE_IS_ANONYMOUS = 6;
    public const RESULT_PASSWORD_NOT_EQUAL   = 7;
    public const RESULT_INVALID_SECURE_HASH  = 8;

    /**
     * Session var name to keep the secret token
     */
    public const SESSION_SECURE_HASH_CELL = 'secureHashCell';

    /**
     * Default hash algorithm
     */
    public const DEFAULT_HASH_ALGO = 'SHA512';

    /**
     * Max count of login attempt
     */
    public const MAX_COUNT_OF_LOGIN_ATTEMPTS = 5;

    /**
     * Time of lock login (sec)
     */
    public const TIME_OF_LOCK_LOGIN = 120;

    /**
     * Recent login cookie name
     */
    public const RECENT_LOGIN_COOKIE_NAME = 'recent_login';

    /**
     * Auth token argument name
     */
    ...
    {
        if (\Includes\Utils\ConfigParser::getOptions(['installer_details', 'shared_secret_key'])) {
            $result = hash_hmac(
                'sha512',
                $password,
                (string) \Includes\Utils\ConfigParser::getOptions(['installer_details', 'shared_secret_key'])
            );
        } else {
    ...
    public function loginProfile(Profile $profile, $withCells = true)
    {
        ...
        if ($result) {
            ...
            Session::getInstance()->setProfile($profile);
     ...
     * @return boolean
     */
    private function hasLoginLock($profile)
    {
    ...
    public function login($login, $password, $secureHash = null)
    {
        ...
        if (!empty($login) && !empty($password)) {
            [$profile, $result] = $this->checkLoginPassword($login, $password, $secureHash);

            if ($result === static::RESULT_INVALID_SECURE_HASH) {
                // TODO - potential attack; send the email to admin
                throw new FatalException('Trying to log in using an invalid secure hash string.');
            }
            ...
            if (isset($profile) && $result === static::RESULT_PASSWORD_NOT_EQUAL) {
                ...
                if (static::MAX_COUNT_OF_LOGIN_ATTEMPTS <= $profile->getCountOfLoginAttempts()) {
    ...
    public function getProfile($profileId = null)
    {
        if (isset($profileId)) {
            ...
            if (!$this->profile['isInitialized']) {
                ...
                /** @var Profile|null $entity */
                $entity = null;

                if ((bool) $this->getOperatingAs() && !\XLite::isAdminZone()) {
                ...
                if (
                    ...
                    && (
                        $entity->isDisabled()
                        || (
                            isset(Session::getInstance()->salt)
                            && Session::getInstance()->salt !== $entity->getSalt()
                            && !$this->getOperatingAs()
                        )
                        || (
                            $this->isAdmin($entity)
                            && Session::getInstance()->isAdminSessionExpired()
                        )
                    )
                ) {
                ...
                if ($entity && \XLite::isAdminZone() && $this->isAdmin($entity)) {
     ...
     *
     * @param Profile|null $profile User profile OPTIONAL
     *
     * @return boolean
     */
    ...
    public function getAccessLevel($type)
    {
        return in_array($type, $this->getUserTypes())
            ? call_user_func([$this, 'get' . $type . 'Accesslevel'])
            : null;
    }
    ...
    {
        return array_map([$this, 'getAccessLevel'], $this->getUserTypes());
    }
    ...
    public function isAuthorized(\XLite\Base $resource)
    {
        ...
        if (!method_exists($resource, 'getAccessLevel')) {
            throw new FatalException('Auth::isAuthorized(): Authorization failed: resource invalid');
        }
    ...
    {
        $this->profile = ['isInitialized' => false, 'object' => null];
    }
    ...
    protected function __construct()
    {
        ...
        if ($token = $this->getRequestAuthToken()) {
    ...
    public function isPermissionAllowedOr($code)
    {
        ...
        return $profile && call_user_func_array([$profile, __METHOD__], func_get_args());
    }
    ...
    {
        return Config::getInstance()->Internal->shop_closed == 'Y';
    }
    ...
    {
        return Request::getInstance()->{self::AUTH_TOKEN};
    }
     ...
     * @return string|null
     */
    public function createToken(Profile $profile): ?string
    {
        return AuthToken::generate([
            'profile_id' => $profile->getProfileId(),
            'exp'        => AuthToken::getDefaultExpirationTime()
        ]);
    }
     ...
     * @return bool
     */
    public function loginByToken($token): bool
    {
        try {
            $data = AuthToken::decode($token);

            $result = $data->profile_id
                && $this->getStoredProfileId() !== $data->profile_id
                && ($profile = Database::getRepo('XLite\Model\Profile')->find($data->profile_id))
                && $this->loginProfile($profile);

            if ($result) {
                Session::getInstance()->set('profile_id', $profile->getProfileId());
                Session::getInstance()->set('salt', $profile->getSalt());
            }
        } catch (\Exception $e) {
            TopMessage::addError('Login by token is failed');
            $this->getLogger()->error($e->getMessage(), ['trace' => $e->getTrace()]);
        }
