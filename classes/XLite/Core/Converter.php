...
class Converter extends \XLite\Base\Singleton
{
    ...
    public const MAX_INT_32 = 2147483647;
    public const MAX_FLOAT_32 = 2.15e+9;
     ...
     * @var array
     */
    protected static $charsetsTable = [
        'ru' => 'WINDOWS-1251',
    ];
    ...
    /**
     * @deprecated use \Includes\Utils\Converter::convertToLowerCamelCase
     */
    public static function convertToCamelCase($string)
    {
        return \Includes\Utils\Converter::convertToUpperCamelCase($string);
    }

    /**
     * @deprecated use \Includes\Utils\Converter::convertFromCamelCase
     */
    public static function convertFromCamelCase($string)
    {
        return \Includes\Utils\Converter::convertFromCamelCase($string);
    }
    ...
    {
        $target = \Includes\Utils\Converter::convertToUpperCamelCase($target);

        // Initialize cache for prod mode only
        if (!LC_DEVELOPER_MODE && static::$cacheControllers === null) {
            static::$cacheControllers = static::getSystemCacheDriver()->fetch('controllers');
            if (!is_array(static::$cacheControllers)) {
                static::$cacheControllers = [];
            }
        }

        // Get controller from cache
        $class = static::$cacheControllers[$zone . '.' . $target] ?? null;

        if (
            !$class
            || !class_exists($class)
        ) {
            // Get core controller from zone
            $class = "XLite\\Controller\\{$zone}\\{$target}";

            if (!class_exists($class)) {
                // Get common core controller
                $class = "XLite\\Controller\\{$target}";
            }
            ...
            if (!class_exists($class)) {
                ...
                // Source path used in dev mode to get immediately created class
                $base = LC_DEVELOPER_MODE ? LC_DIR_MODULES : LC_DIR_CACHE_CLASSES;
                $path = "{$base}*/*/" . (LC_DEVELOPER_MODE ? 'src/' : '') . "Controller/{$zone}/{$target}.php";

                $list = glob($path);
                if ($list) {
                    $length = strlen($base);
                    foreach ($list as $path) {
                        $suggestion = str_replace(['/src/', '/'], ['\\', '\\'], substr($path, $length, -4));

                        if (
                            class_exists($suggestion)
                            && !(new \ReflectionClass($suggestion))->isAbstract()
                        ) {
                            $class = $suggestion;
                            break;
                        }
    ...
    public static function buildURL(
        ...
        $cuFlag           = $forceCu
            ?? (
                LC_USE_CLEAN_URLS
                && (
                    (!\XLite::isAdminZone() && $interface !== \XLite::getAdminScript())
                    || $buildCuInAdminZone
                )
            );
        ...
        return \XLite\Core\URLManager::getShopURL($result);
    }
     ...
     * @return string
     */
    public static function buildFullURL($target = '', $action = '', array $params = [], $interface = null, $forceCu = null, $buildCuInAdminZone = false)
    {
     ...
     * @return string
     */
    public static function buildPersistentAccessURL(\XLite\Model\AccessControlCell $acc, $target = '', $action = '', array $params = [], $interface = null)
    {
        $returnData = [
            'target' => $target,
            'action' => $action,
     ...
     * @return string
     */
    public static function buildCleanURL($target = '', $action = '', array $params = [])
    {
    ...
    public static function getTimeZone($timeZone = null)
    {
        ...
        foreach ($zones as $zone) {
            try {
                ...
                Logger::getLogger('xlite')->notice($e->getMessage());
            }
        }
    ...
    public static function getAvailableDateFormats()
    {
        return [
            ...
            '%b %e, %Y' => [
                'jsFormat'   => 'M dd, yy',
                'phpFormat'  => 'M d, Y',
                'icuFormat'  => 'MMM dd, yyyy',
                'userFormat' => static::formatDate(static::time(), '%b %e, %Y'),
            ],
            '%A, %B %e, %Y'  => [
                'jsFormat'   => 'DD, MM dd, yy',
                'phpFormat'  => 'l, F d, Y',
                'icuFormat'  => 'EEEE, MMMM dd, yyyy',
                'userFormat' => static::formatDate(static::time(), '%A, %B %e, %Y'),
            ]
        ];
    }
    ...
    protected static function getStrftime($format, $base = null)
    {
        ...
        $date = $base !== null ? strftime($format, $base) : strftime($format);
    ...
    {
        [$size, $suffix] = \Includes\Utils\Converter::formatFileSize($size);
    ...
    public static function convertShortSizeToHumanReadable($size)
    {
        ...
        return \XLite\Core\Translation::lbl($label, ['value' => $size]);
    }
    ...
    public static function detectLocaleByCode($code)
    {
        ...
        foreach (static::getDetectLocaleMethods() as $m) {
            $method = 'detectLocaleBy' . \Includes\Utils\Converter::convertToUpperCamelCase($m);
    ...
    {
        $code2 = ($code == 'en' ? 'US' : strtoupper($code));
    ...
    public static function langToLocale($code)
    {
        ...
        return $predefinedLocales[$code] ?? $code;
    }
    ...
    public static function searchClosestLocale($code, $locales)
    {
        ...
        if ($country) {
            ...
            $country = ($code == 'en' ? 'US' : strtoupper($code));
        }
    ...
    /**
     * Allow refresh setLocale
     *
     * @return void
     */
    public static function resetLocaleSet()
    {
