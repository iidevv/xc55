...
class ShippingStatusExistsRule implements RuleInterface
{
    ...
    {
        $invalid = $this->getOrdersWithoutShippingStatus();
        ...
        if ($invalid) {
            $message = \XLite\Core\Translation::getInstance()->translate(
                ...
                [
                    ...
                    'another_model' => 'shippingStatus (XLite\Model\Order\Status\Shipping)',
                ]
            );
     ...
     * @return array
     */
    protected function getOrdersWithoutShippingStatus()
    {
        /** @var AQueryBuilder $qb */
        $qb = $this->repo->createPureQueryBuilder('o');

        $qb->andWhere('o.shippingStatus IS NULL');
