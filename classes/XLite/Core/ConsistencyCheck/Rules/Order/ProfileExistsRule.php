...
class ProfileExistsRule implements RuleInterface
{
    ...
    {
        $invalid = $this->getOrdersWithoutProfiles();
        ...
        if ($invalid) {
            $message = \XLite\Core\Translation::getInstance()->translate(
                ...
                [
                    ...
                    'another_model' => 'profile (XLite\Model\Profile)',
                ]
            );
     ...
     * @return array
     */
    protected function getOrdersWithoutProfiles()
    {
        /** @var AQueryBuilder $qb */
        $qb = $this->repo->createPureQueryBuilder('o');

        $qb->andWhere('o.profile IS NULL');
