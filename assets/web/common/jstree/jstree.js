		...
		select_node: function (obj, supress_event, prevent_open, e) {
			...
			if (!obj.state.selected) {
				...
				if (dom && dom.length) {
					...
					const elem = dom[0];
					const sidebar = document.querySelector('.themetweaker-sidebar');
					if (sidebar && elem) {
						const elemRect = elem.getBoundingClientRect();
						const sidebarRect = sidebar.getBoundingClientRect();
						const space = 20;
						let shouldScrollBy = 0;
						if (elemRect.bottom > sidebarRect.bottom) {
							shouldScrollBy = elemRect.bottom - sidebarRect.bottom + space;
						} else if (elemRect.top < sidebarRect.top) {
							shouldScrollBy = elemRect.top - sidebarRect.top - space;
						}
						if (shouldScrollBy !== 0) {
							sidebar.scrollBy(0, shouldScrollBy);
						}
					}
				}
				/**
