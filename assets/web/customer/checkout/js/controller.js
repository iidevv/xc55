...
{
  xcart.bind('checkout.main.postprocess', _.bind(this.assignHandlers, this))
...
CheckoutView.prototype.assignHandlers = function(event, state)
{
  if (state.isSuccess) {
    ...
    this.base.find('form .ToS-checkbox a').click(_.bind(this.handleOpenTerms, this));
  }
};
...
{
  var addressId = xcart.getValueFromClass(event.currentTarget, 'address');
  if (addressId) {
...
CheckoutView.prototype.handleCheckoutBlock = function()
{
  this.base.find('button.place-order')
    ...
    .addClass('disabled')
    .attr('title', xcart.t('Order can not be placed because not all required fields are completed. Please check the form and try again.'));
};
...
{
  this.shade(xcart.t('Transaction is in progress'));
};
...
CheckoutView.prototype.handleChange = function(event)
{
  ...
  xcart.trigger('checkout.common.anyChange', this);
};
