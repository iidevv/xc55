...
var OrdersListItem = ALoadable.extend({
  constructor: function OrdersListItem(base) {
    this.widgetTarget = xcart.getCommentedData(base, 'widgetTarget');
    this.widgetClass = xcart.getCommentedData(base, 'widgetClass');
    this.widgetParams = xcart.getCommentedData(base, 'widgetParams');
  ...
  postprocess: function(isSuccess, initial) {
    if (!initial && (this.visible || this.openAtStart)) {
      ...
      xcart.autoload(ConfirmDeliveryButton, '.confirm-delivery');
    }
