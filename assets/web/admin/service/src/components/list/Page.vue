...
export default {
  ...
  computed: {
    ...
    pageFilter() {
      return Object.assign({}, this.defaultFilter, this.filter, this.queryFilter);
    },
    tooltipText() {
      return this.type === 'tile'
  ...
  methods: {
    changePageHandler(page) {
      this.$store.commit(
        'modulesData/SET_FILTER',
        Object.assign({}, this.defaultFilter, this.filter, this.queryFilter, { page }),
      );
    },
    ...
    openModuleDetailsPopup(moduleId) {
      const stateToSet = Object.keys(this.transitions).length
