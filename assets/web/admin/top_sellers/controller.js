...
function TopSellers() {
  ...
  var self = this;
  xcart.bind('update-top-sellers', function (event, params) {
    self.load(params);
  });
}
...
TopSellers.prototype.postprocess = function (isSuccess, initial) {
  if (isSuccess) {
    $('.top-sellers-selectors.period-selectors a', this.base).click(function () {
      xcart.trigger('update-top-sellers', {time_interval: $(this).data('interval')});
      return false;
    });

    $('.top-sellers-selectors.availability-selectors a', this.base).click(function () {
      xcart.trigger('update-top-sellers', {availability: $(this).data('availability')});
      return false;
    });
