    ...
    jQuery('.type-s .values .new').bind(
      ...
      function (event) {
        ...
        if (
          ...
          line.find(':input').each(
            function () {
              if (this.id) {
                ...
                const label = line.get(0).querySelector('label[for="' + previousId + '"]');
                if (label) {
          ...
          xcart.trigger('attributes.modifiers.new', { element: newLine })
          if (autoOption) {
...
function changeModifiers(p) {
  ...
  xcart.trigger('attributes.modifiers.change', { element: p })
}
  ...
  line.find(':input').each(
    function () {
      if (this.id) {
        const oldId = this.id;
        this.id = this.id.replace(/-new-id/, '-n' + idx);
        const label = line.get(0).querySelector('label[for="' + oldId + '"]');
        if (label) {
          label.setAttribute('for', this.id);
        }
      }
...
function setTabIndex() {
  jQuery('.attributes .lines')
    .find('input[type=text], textarea')
    .filter("[name*='[value]'], [name*='name['], [name*='[name]']")
    .each(function (i) {
