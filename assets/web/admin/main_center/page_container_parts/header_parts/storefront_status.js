...
StorefrontStatusView.prototype.initialize = function()
{
    ...
    _.bind(
      function(event) {
        ...
        if (!this.blocked && this.userConfirm()) {
          ...
          result = xcart.get(jQuery(event.currentTarget).attr('href'));
        }
  ...
  xcart.bind('switchstorefront', _.bind(this.handleSwicthStorefront, this));
}
...
StorefrontStatusView.prototype.userConfirm = function()
{
  ...
  if (toggler.hasClass('on')) {
    result = confirm(xcart.t('Do you really want to close storefront?'));
  }
...
xcart.autoload('StorefrontStatusView');
