...
{
  xcart.bind('forceMpAction', _.bind(this.forceMpAction, this));
...
{
  return (this.forceMpActions || 0 < xcart.getCommentedData(base, 'hasPendingActions').length)
    && 0 < base.length
...
QueryMarketplace.prototype.scheduleQuery = function(base)
{
  ...
  setTimeout(
    function() {
      ...
      var suffix = '';
      if (xcart.getCommentedData(base, 'parentTarget').length) {
        suffix = '&parentTarget=' + xcart.getCommentedData(base, 'parentTarget');
      }
...
QueryMarketplace.prototype.processSuccess = function(data)
{
  if (data && 0 < data.actions.length) {
    for (i = 0; i < data.actions.length; i++) {
      xcart.trigger('mp_event_' + data.actions[i], data);
    }
  }
}

xcart.autoload(QueryMarketplace);
