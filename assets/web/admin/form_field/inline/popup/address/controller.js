...
function OrderAddressView()
{
  ...
  xcart.bind('afterPopupPlace', _.bind(this.handlePopupOpen, this));
}
...
OrderAddressView.prototype.handlePopupOpen = function()
{
  ...
  if (box.length > 0) {
    ...
    xcart.bind('popup.close', _.bind(this.handleClosePopup, this));
...
OrderAddressView.prototype.syncFieldValue = function(from, to, placeholder)
{
  ...
  // Unlock billing address
  if (to.parents('.inline-field').hasClass('profile-billingAddress')) {
    to.parents('.inline-field').removeClass('same-as-shipping');
    to.parents('form').find('input[name="billingAddress[same_as_shipping]"]').val('');
  }
}
...
xcart.autoload(OrderAddressView);
