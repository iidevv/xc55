...
CommonElement.prototype.handlers.push(
  {
    ...
    handler: function () {
      ...
      $wrapper.get(0).model_selector_options = {
        getter: xcart.getCommentedData($wrapper, 'getter'),
        min_count: xcart.getCommentedData($wrapper, 'min_count'),
        spacer_path: xcart.getCommentedData($wrapper, 'spacer_path')
      };
      ...
      var doSetModelAsNotSelected = function () {
        ...
        xcart.trigger(elementNamespace + '.not-selected', {
          element: $element
        });
      };
      ...
      var doSelectValue = function ($this) {
        ...
        xcart.trigger(elementNamespace + '.before-selected', {element: $element});
      ...
      var doPopulateList = function (data) {
        ...
        jQuery(data).each(function (index, elem) {
          jQuery('ul.items-list', $wrapper).append(
            `<li class="adding"><span class="text"></span><img src="${$wrapper.get(0).model_selector_options.spacer_path}" class="right-fade" alt="" /></li>`
          );
      ...
      this.$element.bind('keyup', function (event) {
        ...
        if ($element.val().length < $wrapper.get(0).model_selector_options.min_count) {
          if ($element.val().length > 0) {
            jQuery('.enter-more-characters', $wrapper).html(xcart.t(
