...
OrderInfoForm.prototype.initialize = function()
{
  ...
  xcart.bind('stickypanel.check.button.enable', _.bind(this.handleCheckEnableButton, this));
  xcart.bind('stickypanel.check.button.disable', _.bind(this.handleCheckDisableButton, this));
  xcart.bind('stickypanel.markAsChanged', _.bind(this.handleUpdateButtonsState, this));
  xcart.bind('stickypanel.unmarkAsChanged', _.bind(this.handleUpdateButtonsState, this));

  xcart.bind('recalculateOrder', _.bind(this.handleRecalculateOrderEvent, this));
  ...
  xcart.bind('list.model.table.order.items.newLineCreated', _.bind(this.handleCreateNewLine, this));
...
OrderInfoForm.prototype.isElementAffectRecalculate = function(element)
{
  return !jQuery(element).hasClass('not-affect-recalculate')
    ...
    && element.name != xliteConfig.form_id_name;
}
...
OrderInfoForm.prototype.handleRestore = function(event)
{
  ...
  xcart.trigger('order.modifier.changed', { modifier: box });
...
OrderInfoForm.prototype.updateModifierElement = function(value, code)
{
  ...
  if (box.length) {
    var str = xcart.numberToString(value, '.', '', parseInt(jQuery('.order-info').data('e')));
...
{
  if (
    !this.canEnableButton(state.button)
    || this.useOriginalBlockSubmitButtonLogic(state)
  ) {
    state.state = true;
    state.inverse = false;
...
// don't unblock submit button despite the logic from https://xcn.myjetbrains.com/youtrack/issue/XCN-4532 . Use default 'block submit button' logic for the native 'submit' JS event
OrderInfoForm.prototype.useOriginalBlockSubmitButtonLogic = function (state) {
  var button = state.button
  return jQuery(button).hasClass('submit') && this.elementsState.needSave && !this.elementsState.needRecalculate && !jQuery(button).hasClass('recalculate')
    && button.jqueryEvent && button.jqueryEvent.originalEvent && button.jqueryEvent.originalEvent.type
    && button.jqueryEvent.originalEvent.type === 'submit' && (typeof SubmitEvent !== 'undefined') && button.jqueryEvent.originalEvent instanceof SubmitEvent
    && !this.base.get(0).isBgSubmitting && state.state;
}
