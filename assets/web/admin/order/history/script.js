...
OrderEventDetails.prototype.handleSwitch = function(event)
{
  if (this.base.hasClass('in')) {
    ...
    jQuery('.order-info .title-box .history a').html(xcart.t('View order history'));

  } else {
    this.base.collapse('show');
    jQuery('.order-info .title-box .history a').html(xcart.t('Hide order history'));
  }

  return false;
}

xcart.autoload('OrderEventDetails');
