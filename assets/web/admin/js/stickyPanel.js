...
StickyPanel.prototype.process = function()
{
  ...
  if (!this.isModal()) {
    ...
    xcart.bind('stickyPanelReposition', _.bind(this.reposition, this));
  }
...
{
  if (jQuery(button).closest('.force-change-password-section').length) {
    return;
  }
...
StickyPanel.prototype.blockSubmitButton = function (event)
{
  this.getSubmitButton().each(
    _.bind(
      function (_event, index, button) {
        button.jqueryEvent = _event
        this.disableButton(button);
      },
      this, event
    )
