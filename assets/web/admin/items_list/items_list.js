...
//TODO change to getCommentedData() -> also in templates
  return this.process('pageId', xcart.getValueFromClass(handler, 'page'));
};
...
ItemsList.prototype.initialize = function(elem, urlparams, urlajaxparams)
{
  ...
  if (typeof(urlparams) == 'undefined') {
    ...
    this.container = elem;
    this.params = xcart.getCommentedData(elem);
  ...
  if (this.container.length) {
    ...
    const isItemsListEmpty = !container.querySelector('.table-wrapper') || container.querySelector('.table-wrapper.empty');
    container.parentNode.querySelectorAll('.hide-if-empty-list').forEach(
      function(element) {
        CommonForm.prototype.findParentCellForElementOnStickyPanel(element)
          .classList.toggle('list-is-empty', isItemsListEmpty);
      }
    );
...
{
  var urlparams = xcart.getCommentedData(this.container, 'urlparams'),
    urlajaxparams = xcart.getCommentedData(this.container, 'urlajaxparams');
...
ItemsList.prototype.addListeners = function()
{
  ...
  xcart.microhandlers.runAll(this.container);
...
xcart.bind('confirm_reject', function(event, el) {
  setTimeout(function () {
    jQuery(el).blur().tooltip('hide');
  }, 300);
});
