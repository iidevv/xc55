...
OrderItemsList.prototype.initialize = function(elem, urlparams, urlajaxparams)
{
  ...
  if (result) {
    ...
    xcart.bind('order.item.changed', _.bind(this.handleItemChanged, this));

    xcart.bind('order.items.changed', _.bind(this.handleItemsChanged, this));

    xcart.bind('model-selector.product.before-selected', _.bind(this.handleBeforeProductModelSelect, this));
    xcart.bind('model-selector.product.selected', _.bind(this.handleProductModelSelect, this));
    ...
    xcart.bind('order.itemAttributes.changed', _.bind(this.handleOrderItemAttributesChanged, this));

    xcart.bind('recalculateitem', _.bind(this.handleRecalculateItem, this));
  }
...
OrderItemsList.prototype.processLine = function(idx, line)
{
  ...
  line.find('.edit-options-link a.open-popover').popover({
    ...
    sanitize: this.getPopoverWhiteListToSanitize()
  });
...
OrderItemsList.prototype.getPopoverWhiteListToSanitize = function()
{
  let whiteList = $.fn.tooltip.Constructor.DEFAULTS.whiteList;
  whiteList.option = ['value', 'selected', 'data-init-selected', 'data-modifier-price', 'data-modifier-weight'];
  const elmOptions = ['name', 'data-attribute-id'];
  _.extend(whiteList.select, elmOptions);
  _.extend(whiteList.textarea, elmOptions);
  return whiteList;
}
...
OrderItemsList.prototype.handlePopoverCheckboxChange = function(event)
{
  ...
  box.find('.av-' + elm.data('attribute-id'))
    .html(checked ? xcart.t('Yes') : xcart.t('No'));
...
OrderItemsList.prototype.handlePriceChange = function(event)
{
  ...
  xcart.trigger('order.item.changed', { line: field.parents('tr') });
}
...
OrderItemsList.prototype.handleRestorePrice = function(event)
{
  ...
  xcart.trigger('order.item.changed', { 'line': line });
...
OrderItemsList.prototype.handleItemChanged = function(event, data)
{
  ...
  var total = xcart.round(price * qty, this.e);
  line.find('td.cell.total .value').data('value', total);
  line.find('td.cell.total .value').html(xcart.numberToString(total, '.', '', this.e));
  ...
  xcart.trigger('order.items.changed');
}
...
OrderItemsList.prototype.handleItemsChanged = function(event)
{
  ...
  sum = xcart.numberToString(sum, '.', '', this.e).split(/\./, 2);
...
OrderItemsList.prototype.handleProductModelSelect = function(event, data)
{
  ...
  input.val(xcart.numberToString(data.data.selected_price, '.', '', this.e));
...
OrderItemsList.prototype.handleNewItemCreated = function(event, data)
{
  if (jQuery(data.line).parents('.order-items').length) {
    ...
    jQuery(data.line).find('td.cell.price .inline-field :input,td.cell.amount .inline-field :input').change(
      function(event) {
        xcart.trigger('order.item.changed', { line: data.line });
      }
    );
...
OrderItemsList.prototype.handleOptionsPopupSubmit = function(event)
{
  ...
  xcart.trigger('order.itemAttributes.changed', { line: this.lastOptionContainer.parents('tr').eq(0) });
...
OrderItemsList.prototype.saveOptionCheckbox = function(idx, elm)
{
  ...
  this.lastOptionContainer
    ...
    .html(elm.is(':checked') ? xcart.t('Yes') : xcart.t('No'));
}
...
OrderItemsList.prototype.updateCell = function(line, data)
{
  ...
  price = xcart.numberToString(price, '.', '', this.e);
  ...
  if (td.length) {
    ...
    td.find('.restore-orig-price>span').attr(
      'title',
      xcart.t('Current price for the selected configuration and quantity: X', {'price': price})
    );
...
OrderItemsList.prototype.handleRemoveLine = function(event)
{
  setTimeout(
    function() {
      xcart.trigger('order.item.changed', { line: jQuery(event.currentTarget).parents('tr') });
    },
    100
  );
}
